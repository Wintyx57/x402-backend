...
async function parseSpec(source) {
  ...
  if (source.url) {
    ...
    try {
      ...
      // Parse YAML manually — SwaggerParser.parse() treats strings as file paths
...
/**
 * Detect if an OpenAPI spec originates from RapidAPI.
 * Checks for x-rapidapi-info extension and .p.rapidapi.com server URLs.
 * @param {object} spec - Parsed OpenAPI/Swagger spec
 * @returns {object|null} RapidAPI metadata or null if not a RapidAPI spec
 */
function detectRapidAPI(spec) {
  ...
  // Extract server URL: OpenAPI 3.x (servers[]) or Swagger 2.0 (host + basePath)
