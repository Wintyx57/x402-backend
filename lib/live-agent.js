...
async function callApiWithPayment(api) {
    ...
    try {
        ...
        // Note: paymentMiddleware consumes the hash immediately (no deferClaim on wrappers).
        // If the upstream API fails, the hash is already consumed — do NOT retry.
        ...
        let parsed = null;
        try { parsed = JSON.parse(paidBody); } catch { /* not JSON */ }

        if (paidRes.ok && parsed) {
            result.data = parsed;
        } else if (parsed && parsed.success !== undefined) {
            // Got JSON but non-200 — upstream API might have partial data
            result.data = parsed;
            result.error = `API returned ${paidRes.status}`;
        } else {
            // Real failure — log status + snippet of body for debugging
            const snippet = paidBody.slice(0, 120);
            result.error = `Paid call returned ${paidRes.status}: ${snippet}`;
        }
    } catch (err) {
