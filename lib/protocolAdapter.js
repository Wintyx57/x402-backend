...
function detectX402V2(headers) {
  ...
  const isStandard = parsed.x402Version && accept && accept.asset && accept.scheme === 'exact';
  result.protocolType = isStandard ? 'x402-standard' : 'direct';
  if (accept) {
    result.payTo = accept.payTo || accept.address || null;
    result.maxAmountRequired = accept.maxAmountRequired ? String(accept.maxAmountRequired) : null;
...
function detectX402V1(body) {
  ...
  const isStandard = body.x402Version && accept && accept.asset && accept.scheme === 'exact';
  result.protocolType = isStandard ? 'x402-standard' : 'direct';
  if (accept) {
    result.payTo = accept.payTo || accept.address || null;
    result.maxAmountRequired = accept.maxAmountRequired ? String(accept.maxAmountRequired) : null;
...
function buildUniversalProofHeaders(normalized, txHash, chainKey, agentWallet) {
  ...
  return {
    headers: {
      ...
      'PAYMENT-SIGNATURE': Buffer.from(JSON.stringify(v2Payload)).toString('base64'),
