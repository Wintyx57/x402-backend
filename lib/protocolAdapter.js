...
const USDC_CONTRACTS = new Set([
  ...
  "0x85889c8c714505e0c94b30fcfcf64fe3ac8fcb20", // SKALE on Base
]);
...
function detectX402V2(headers) {
  ...
  const isStandard =
    parsed.x402Version && accept && accept.asset && accept.scheme === "exact";
  result.protocolType = isStandard ? "x402-standard" : "direct";
  if (accept) {
    result.payTo = accept.payTo || accept.address || null;
    result.maxAmountRequired = accept.maxAmountRequired
      ? String(accept.maxAmountRequired)
      : null;
    result.maxTimeoutSeconds = accept.maxTimeoutSeconds || null;
    result.extra = accept.extra || null;
    result.resource = accept.resource || null;
  }
  ...
  if (params.request) {
    try {
      const decoded = JSON.parse(
        Buffer.from(params.request, "base64").toString("utf-8"),
      );
      ...
      /* request parsing is best-effort */
    }
  }
...
function detectStripe402(headers) {
  ...
  const isStripe =
    parsed.stripePublishableKey || (parsed.price != null && parsed.currency);
...
function detectX402V1(body) {
  ...
  const isStandard =
    body.x402Version && accept && accept.asset && accept.scheme === "exact";
  result.protocolType = isStandard ? "x402-standard" : "direct";
  if (accept) {
    result.payTo = accept.payTo || accept.address || null;
    result.maxAmountRequired = accept.maxAmountRequired
...
function detectX402Variant(body) {
  ...
  const hasX402Entry = body.accepts.some(
    (a) => a.x402 === true || (a.address && a.amount),
  );
  ...
  if (!accept)
    accept = body.accepts.find(
      (a) => (a.currency || "").toUpperCase() === "USDC",
    );
  ...
  result.asset = accept.asset || null;
  result.currency =
    accept.currency || (accept.asset && isUSDC(accept.asset) ? "USDC" : null);
...
function detectX402Bazaar(body) {
  if (!body.payment_details || typeof body.payment_details !== "object")
    return null;
...
function buildProofHeaders(normalized, txHash, chainKey, agentWallet) {
  ...
  if (!txHash) {
    return {
      headers: null,
      supported: false,
      message: "Cannot build proof: txHash is required",
    };
  }
  ...
  switch (format) {
    ...
    case "x402-v1": {
      // Send BOTH Coinbase x402 format AND simple headers.
      // External providers vary — some parse X-PAYMENT base64, others check X-Payment-TxHash.
      // Sending both maximizes compatibility (headers don't conflict).
      const payload = {
        x402Version: 1,
    ...
    case "l402-protocol": {
      return {
        ...
        supported: false,
        message:
          "L402-protocol requires multi-step flow (offer selection + payment request)",
      };
    }
...
// ─── Layer 2b: Universal Proof Builder (for external APIs) ──────────────────

/**
 * Build ALL known proof header formats simultaneously for maximum compatibility
 * with any 402 provider. Extra headers are harmless — providers ignore unknown ones.
 *
 * Used for split_platform mode (external APIs) where we don't know what format
 * the provider expects. Internal APIs use buildProofHeaders() instead.
 *
 * @param {object} normalized - Result of normalize402()
 * @param {string} txHash - On-chain transaction hash
 * @param {string} chainKey - Chain key (base, skale, polygon)
 * @param {string} agentWallet - Agent wallet address (0x...)
 * @returns {{ headers: object, supported: boolean, message: string|null }}
 */
function buildUniversalProofHeaders(normalized, txHash, chainKey, agentWallet) {
  if (!txHash) {
    return {
      headers: null,
      supported: false,
  ...
  const chain = CHAIN_KEY_MAP[chainKey] || chainKey;
  const scheme = normalized?.scheme || "exact";
  const network = normalized?.chain || chain;

  // x402 v1 payload (Coinbase standard)
  const v1Payload = {
    x402Version: 1,
    scheme,
    network,
  ...
  return {
    headers: {
      // ── Coinbase x402 v1 format (base64 JSON) ──
      "X-PAYMENT": Buffer.from(JSON.stringify(v1Payload)).toString("base64"),
      // ── x402 v2 format (base64 JSON) ──
      "PAYMENT-SIGNATURE": Buffer.from(JSON.stringify(v2Payload)).toString(
        "base64",
      ),
      // ── Simple direct headers (raw txHash) ──
      "X-Payment-TxHash": txHash,
      "X-Payment-Chain": chain,
      "X-Payment": txHash,
