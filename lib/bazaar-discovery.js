...
const arxivDiscovery = declareDiscoveryExtension({
  ...
  output: {
    example: {
      ...
      results_count: 5,
      results: [{ title: 'Attention Is All You Need', authors: ['Ashish Vaswani'], published: '2017-06-12', id: 'http://arxiv.org/abs/1706.03762v7' }]
    }
  }
});
...
const crossrefDiscovery = declareDiscoveryExtension({
  ...
  output: {
    example: {
      success: true,
      results: [{ title: 'Deep Learning in Healthcare', doi: '10.1234/example', citations: 42 }]
    }
  }
});
...
const tvshowDiscovery = declareDiscoveryExtension({
  ...
  output: {
    example: {
      success: true,
      results: [{ name: 'Breaking Bad', rating: 9.5, genres: ['Drama', 'Crime'] }]
    }
  }
});
...
const itunesDiscovery = declareDiscoveryExtension({
  ...
  inputSchema: {
    properties: {
      q:     { type: 'string', maxLength: 200 },
      media: { type: 'string', enum: ['music', 'movie', 'podcast', 'audiobook', 'tvShow', 'software', 'ebook', 'all'], default: 'music' }
    },
    required: ['q']
  },
  output: {
...
const animeDiscovery = declareDiscoveryExtension({
  ...
  output: {
    example: {
      success: true,
      results: [{ title: 'Naruto', score: 8.27, episodes: 220, genres: ['Action', 'Adventure'] }]
    }
  }
});
...
const ethGasDiscovery = declareDiscoveryExtension({
  ...
  output: {
    example: {
      success: true,
      chains: { ethereum: { gwei: 25.3 }, base: { gwei: 0.012 }, polygon: { gwei: 30.1 } }
    }
  }
});
...
const colorPaletteDiscovery = declareDiscoveryExtension({
  ...
  inputSchema: {
    properties: {
      seed: { type: 'string', description: 'Optional hex color seed (e.g. ff5733)' }
    },
    required: []
  },
  output: {
...
const openverseDiscovery = declareDiscoveryExtension({
  ...
  output: {
    example: {
      success: true,
      results: [{ title: 'Sunset over mountains', url: 'https://...', license: 'CC BY 2.0' }]
    }
  }
});
...
const emojiDiscovery = declareDiscoveryExtension({
  ...
  output: {
    example: {
      success: true,
      results: [{ name: 'smiling face with open mouth', emoji: '😃', category: 'smileys and people' }]
    }
  }
});
...
const textStatsApiDiscovery = declareDiscoveryExtension({
  bodyType: 'json',
  input: { text: 'The quick brown fox jumps over the lazy dog. It was a beautiful day.' },
  inputSchema: {
    properties: {
...
function getOutputExampleForUrl(serviceUrl) {
    try {
        ...
        if (!disc) return null;
        return disc?.bazaar?.info?.output?.example
            ?? disc?.output?.example
            ?? null;
    } catch {
        return null;
...
module.exports = { discoveryMap, generateDiscoveryForService, getInputSchemaForUrl, getMethodForUrl, getOutputExampleForUrl };
