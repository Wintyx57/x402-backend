      ...
      results: [
        {
          ...
          snippet: "Build LLM-powered apps.",
        },
      ],
    },
  },
...
const timestampDiscovery = declareDiscoveryExtension({
  ...
  inputSchema: {
    properties: {
      ts: {
        type: "number",
        description: "Unix timestamp to convert (optional)",
      },
      date: {
        type: "string",
        description: "ISO date string to convert (optional)",
      },
    },
    required: [],
  },
...
const summarizeDiscovery = declareDiscoveryExtension({
  ...
  output: {
    example: {
      summary:
        "AI is transforming every industry by automating tasks and enabling new capabilities.",
      originalLength: 2540,
      summaryLength: 84,
    },
  },
...
const qrcodeGenDiscovery = declareDiscoveryExtension({
  ...
  output: {
    example: {
      success: true,
      image_url:
        "https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=https%3A%2F%2Fx402bazaar.org&format=png",
      data: "https://x402bazaar.org",
      size: 300,
    },
  },
...
const sentimentDiscovery = declareDiscoveryExtension({
  input: {
    text: "This product is absolutely amazing and exceeded all expectations!",
  },
  inputSchema: {
    properties: {
...
const wikipediaDiscovery = declareDiscoveryExtension({
  ...
  output: {
    example: {
      title: "Blockchain",
      extract:
        "A blockchain is a distributed ledger with growing list of records...",
...
const dogsDiscovery = declareDiscoveryExtension({
  ...
  inputSchema: {
    properties: {
      breed: {
        type: "string",
        description: "Dog breed name (optional, random if omitted)",
      },
    },
    required: [],
  },
...
const contractRiskDiscovery = declareDiscoveryExtension({
  bodyType: "json",
  input: {
    text: "This agreement shall be governed by... The contractor shall indemnify...",
  },
  inputSchema: {
    properties: {
      ...
      clauses: [
        {
          ...
          risk: "high",
          explanation: "Broad indemnification clause with no cap.",
        },
      ],
    },
  },
...
const emailParseDiscovery = declareDiscoveryExtension({
  ...
  output: {
    example: {
      ...
      sentiment: "positive",
      summary:
        "Inbound partnership request from Acme Corp interested in API integration.",
    },
  },
});
...
const tableInsightsDiscovery = declareDiscoveryExtension({
  ...
  output: {
    example: {
      ...
      insights: [
        "Revenue grew 54% from Jan to Mar.",
        "Users increased 50% in Q1.",
      ],
...
// ─────────────────────────────────────────────────────────────────────────────
// SCIENCE & SPACE (5 APIs)
// ─────────────────────────────────────────────────────────────────────────────

const arxivDiscovery = declareDiscoveryExtension({
  input: { query: "transformer attention mechanism", max: 5 },
  inputSchema: {
    properties: {
...
const nasaDiscovery = declareDiscoveryExtension({
  input: {},
  ...
  output: {
    example: {
      ...
      date: "2026-03-17",
      media_type: "image",
      url: "https://apod.nasa.gov/apod/image/2603/NGC1566.jpg",
    },
  },
});

const issDiscovery = declareDiscoveryExtension({
  input: {},
  inputSchema: { properties: {} },
  output: {
    example: {
      success: true,
      position: { latitude: 42.3, longitude: -71.1 },
      crew: { count: 7, members: ["Oleg Kononenko", "Tracy Dyson"] },
    },
  },
});

const spacexDiscovery = declareDiscoveryExtension({
  input: { type: "latest" },
  inputSchema: {
    properties: {
      type: { type: "string", enum: ["latest", "upcoming"], default: "latest" },
    },
    required: [],
  },
  output: {
    example: {
      ...
      date_utc: "2026-03-15T04:00:00.000Z",
      launch_success: true,
    },
  },
});

const crossrefDiscovery = declareDiscoveryExtension({
  input: { query: "machine learning healthcare", max: 5 },
  inputSchema: {
    properties: {
      query: { type: "string", maxLength: 300 },
      max: { type: "integer", minimum: 1, maximum: 20, default: 5 },
...
const tvshowDiscovery = declareDiscoveryExtension({
  input: { q: "breaking bad" },
  inputSchema: {
    properties: {
      q: { type: "string", maxLength: 200 },
    },
  ...
  output: {
    example: {
      success: true,
      results: [
        { name: "Breaking Bad", rating: 9.5, genres: ["Drama", "Crime"] },
      ],
    },
  },
});

const booksDiscovery = declareDiscoveryExtension({
  input: { q: "dune frank herbert" },
  inputSchema: {
    properties: {
      q: { type: "string", maxLength: 200 },
...
const itunesDiscovery = declareDiscoveryExtension({
  input: { q: "radiohead", media: "music" },
  inputSchema: {
    properties: {
      q: { type: "string", maxLength: 200 },
      media: {
  ...
  output: {
    example: {
      success: true,
      results: [{ name: "Creep", artist: "Radiohead", genre: "Alternative" }],
    },
  },
});

const animeDiscovery = declareDiscoveryExtension({
  input: { q: "naruto" },
  inputSchema: {
    properties: {
      q: { type: "string", maxLength: 200 },
...
const btcAddressDiscovery = declareDiscoveryExtension({
...
const ethGasDiscovery = declareDiscoveryExtension({
  input: {},
  inputSchema: { properties: {} },
  output: {
    example: {
...
// ─────────────────────────────────────────────────────────────────────────────
// IMAGES & CREATIVE (3 APIs)
// ─────────────────────────────────────────────────────────────────────────────

const catDiscovery = declareDiscoveryExtension({
  input: {},
  inputSchema: { properties: {} },
  output: {
    example: {
      success: true,
      image_url: "https://cataas.com/cat/abc123",
      tags: ["cute", "orange"],
    },
  },
});

const colorPaletteDiscovery = declareDiscoveryExtension({
  input: { seed: "ff5733" },
  inputSchema: {
    properties: {
      seed: {
        type: "string",
        description: "Optional hex color seed (e.g. ff5733)",
      },
    },
    required: [],
  },
  output: {
    example: {
      success: true,
      palette: [{ hex: "#ff5733", rgb: { r: 255, g: 87, b: 51 } }],
    },
  },
});

const openverseDiscovery = declareDiscoveryExtension({
  input: { q: "sunset mountain" },
  inputSchema: {
    properties: {
      q: { type: "string", maxLength: 200 },
...
const publicIpDiscovery = declareDiscoveryExtension({
  input: {},
  inputSchema: { properties: {} },
  output: {
    example: {
...
const uptimeDiscovery = declareDiscoveryExtension({
  input: { url: "https://x402bazaar.org" },
  inputSchema: {
    properties: {
      url: { type: "string", format: "uri" },
...
const robotsDiscovery = declareDiscoveryExtension({
  ...
  output: {
    example: {
      ...
      found: true,
      rules_count: 15,
      sitemaps: ["https://google.com/sitemap.xml"],
    },
  },
});
...
const elevationDiscovery = declareDiscoveryExtension({
  input: { lat: 48.85, lon: 2.35 },
  inputSchema: {
    properties: {
      lat: { type: "number", description: "Latitude (-90 to 90)" },
  ...
  output: {
    example: {
      success: true,
      elevation_meters: 35,
      elevation_feet: 114.8,
    },
  },
});

const timezoneDiscovery = declareDiscoveryExtension({
  input: { lat: 48.85, lon: 2.35 },
  inputSchema: {
    properties: {
      lat: { type: "number", description: "Latitude (-90 to 90)" },
...
// ─────────────────────────────────────────────────────────────────────────────
// FUN & MOCK DATA (3 APIs)
// ─────────────────────────────────────────────────────────────────────────────

const randomUserDiscovery = declareDiscoveryExtension({
  input: {},
  inputSchema: { properties: {} },
  output: {
    example: {
...
const emojiDiscovery = declareDiscoveryExtension({
  input: { q: "smile" },
  inputSchema: {
    properties: {
...
const pokemonDiscovery = declareDiscoveryExtension({
  input: { name: "pikachu" },
  inputSchema: {
    properties: {
      name: { type: "string" },
...
const detectLangDiscovery = declareDiscoveryExtension({
  bodyType: "json",
...
const textStatsApiDiscovery = declareDiscoveryExtension({
  bodyType: "json",
  input: {
...
const metarDiscovery = declareDiscoveryExtension({
  input: { icao: "KJFK" },
  inputSchema: {
    properties: {
...
const malwareCheckDiscovery = declareDiscoveryExtension({
  input: { url: "https://example.com" },
  inputSchema: {
    properties: {
      url: { type: "string", format: "uri" },
...
const discoveryMap = {
  ...
  // SCIENCE & SPACE
  "/api/arxiv": arxivDiscovery,
  "/api/nasa": nasaDiscovery,
  "/api/iss": issDiscovery,
  "/api/spacex": spacexDiscovery,
  "/api/crossref": crossrefDiscovery,

  // MEDIA & ENTERTAINMENT
  "/api/tvshow": tvshowDiscovery,
  "/api/books": booksDiscovery,
  "/api/itunes": itunesDiscovery,
  "/api/anime": animeDiscovery,

  // BLOCKCHAIN
  "/api/btc-address": btcAddressDiscovery,
  "/api/eth-gas": ethGasDiscovery,

  // IMAGES & CREATIVE
  "/api/cat": catDiscovery,
  "/api/color-palette": colorPaletteDiscovery,
  "/api/openverse": openverseDiscovery,

  // DEV TOOLS
  "/api/public-ip": publicIpDiscovery,
  "/api/uptime": uptimeDiscovery,
  "/api/robots": robotsDiscovery,

  // LOCATION
  "/api/elevation": elevationDiscovery,
  "/api/timezone": timezoneDiscovery,

  // FUN & MOCK DATA
  "/api/random-user": randomUserDiscovery,
  "/api/emoji": emojiDiscovery,
  "/api/pokemon": pokemonDiscovery,

  // TEXT & NLP
  "/api/detect-lang": detectLangDiscovery,
  "/api/text-stats": textStatsApiDiscovery,
  "/api/metar": metarDiscovery,

  // SECURITY
  "/api/malware-check": malwareCheckDiscovery,
...
const _inputSchemaMap = {
  ...
  // SCIENCE & SPACE
  "/api/arxiv": { required: ["query"] },
  "/api/crossref": { required: ["query"] },
  // MEDIA & ENTERTAINMENT
  "/api/tvshow": { required: ["q"] },
  "/api/books": { required: ["q"] },
  "/api/itunes": { required: ["q"] },
  "/api/anime": { required: ["q"] },
  // BLOCKCHAIN
  "/api/btc-address": { required: ["address"] },
  // IMAGES & CREATIVE
  "/api/openverse": { required: ["q"] },
  // DEV TOOLS
  "/api/uptime": { required: ["url"] },
  "/api/robots": { required: ["url"] },
  // LOCATION
  "/api/elevation": { required: ["lat", "lon"] },
  "/api/timezone": { required: ["lat", "lon"] },
  // FUN & MOCK DATA
  "/api/emoji": { required: ["q"] },
  "/api/pokemon": { required: ["name"] },
  // TEXT & NLP
  "/api/detect-lang": { required: ["text"], method: "POST" },
  "/api/text-stats": { required: ["text"], method: "POST" },
  "/api/metar": { required: ["icao"] },
  // SECURITY
  "/api/malware-check": { required: ["url"] },
};
...
/**
 * Get the output example for a service URL from discoveryMap.
 * Used by response-validator.js for schema validation + quality scoring.
 * @param {string} serviceUrl — full URL like https://x402-api.onrender.com/api/weather
 * @returns {object|null} — the output.example object or null
 */
function getOutputExampleForUrl(serviceUrl) {
  try {
    const path = new URL(serviceUrl).pathname;
    const disc = discoveryMap[path];
    if (!disc) return null;
    return disc?.bazaar?.info?.output?.example ?? disc?.output?.example ?? null;
  } catch {
    return null;
  }
}
...
module.exports = {
  discoveryMap,
  generateDiscoveryForService,
  getInputSchemaForUrl,
  getMethodForUrl,
  getOutputExampleForUrl,
};
