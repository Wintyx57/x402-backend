...
// sends USDC back to the agent wallet on-chain, from a separate refund wallet.
//
// Design goals:
//   - Fire-and-forget: processRefund() never throws; errors are logged only.
//   - Lazy init: viem clients created once, on first real use.
//   - Graceful degradation: if REFUND_PRIVATE_KEY is missing, silently return not_configured.
...
// ---------------------------------------------------------------------------
const REFUND_KEY = process.env.REFUND_PRIVATE_KEY || null;
...
function init() {
    ...
    if (!REFUND_KEY) {
        logger.info('Refund', 'Not configured — set REFUND_PRIVATE_KEY to enable auto-refunds');
        return false;
    }
