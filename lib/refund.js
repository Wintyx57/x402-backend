...
// ---------------------------------------------------------------------------
const REFUND_KEY =
  process.env.REFUND_PRIVATE_KEY || process.env.AGENT_PRIVATE_KEY || null;
const DAILY_CAP =
  process.env.REFUND_DAILY_CAP_USDC !== undefined
    ? Number(process.env.REFUND_DAILY_CAP_USDC)
    : 50;
...
function init() {
  ...
  if (!REFUND_KEY) {
    logger.info(
      "Refund",
      "Not configured — set REFUND_PRIVATE_KEY or AGENT_PRIVATE_KEY to enable auto-refunds",
    );
    return false;
  }
  ...
  if (!/^0x[a-fA-F0-9]{64}$/.test(REFUND_KEY)) {
    logger.warn(
      "Refund",
      "REFUND_PRIVATE_KEY is not a valid private key (expected 0x + 64 hex chars)",
    );
    return false;
  }
  ...
  try {
    ...
    logger.info(
      "Refund",
      `Initialized — refund wallet ${_account.address.slice(0, 10)}..., daily cap ${DAILY_CAP} USDC/chain`,
    );
    return true;
  } catch (err) {
...
function getRefundStatus() {
  ...
  const keySource = process.env.REFUND_PRIVATE_KEY
    ? "REFUND_PRIVATE_KEY"
    : "AGENT_PRIVATE_KEY";
  return {
    configured: true,
    ...
    balanceCache: Object.fromEntries(
      Object.entries(_balanceCache).map(([k, v]) => [
        k,
        {
          balance_usdc:
            v.balance !== undefined
              ? (Number(v.balance) / 1e6).toFixed(6)
              : null,
          age_seconds: v.updatedAt
            ? Math.floor((Date.now() - v.updatedAt) / 1000)
            : null,
        },
      ]),
    ),
...
async function processRefund(
  ...
  if (rt) {
    if (
      rt.count >= REPEAT_MAX &&
      Date.now() < rt.lastAt + REPEAT_BLOCK_DURATION
    ) {
  ...
  // 6. Daily cap (use integer math to avoid float precision issues)
  _resetDailyIfNeeded();
  const amountCents = Math.round(amountUsdc * 1e6);
  const capCents = Math.round(DAILY_CAP * 1e6);
  const spentCents = Math.round((_dailySpend[chainKey] || 0) * 1e6);
  if (spentCents + amountCents > capCents) {
    return { refunded: false, reason: "daily_cap_exceeded" };
  }

  // 7. Reserve counters BEFORE async transfer (prevents race conditions)
  //    If transfer fails, we rollback below.
  _updateRateLimit(rlKey);
  _updateRepeatTracker(repeatKey);
  _dailySpend[chainKey] = (_dailySpend[chainKey] || 0) + amountUsdc;

  // 8. Balance check (with cache)
  try {
    const balance = await _getBalance(chainKey);
    const amountRaw = BigInt(amountCents);
    if (balance < amountRaw) {
      _rollbackCounters(rlKey, repeatKey, chainKey, amountUsdc);
      return { refunded: false, reason: "insufficient_balance" };
    }
  } catch (err) {
    logger.error(
      "Refund",
      `Balance check failed on ${chainKey}: ${err.message}`,
    );
    _rollbackCounters(rlKey, repeatKey, chainKey, amountUsdc);
    return { refunded: false, reason: "balance_check_failed" };
  }

  // 9. Execute on-chain transfer
  try {
    const viem = require("viem");
    ...
    logger.info(
      "Refund",
      `Refunded ${amountUsdc} USDC to ${agentWallet.slice(0, 10)}... on ${chainKey} — tx: ${txHash}`,
    );
    return { refunded: true, txHash };
  } catch (err) {
    logger.error(
      "Refund",
      `Transfer failed on ${chainKey} to ${agentWallet.slice(0, 10)}...: ${err.message}`,
    );
    _rollbackCounters(rlKey, repeatKey, chainKey, amountUsdc);
...
function _updateRateLimit(walletKey) {
  ...
  if (rl && Date.now() < rl.resetAt) {
    ...
    _refundRateLimit.set(walletKey, {
      count: 1,
      resetAt: Date.now() + RATE_LIMIT_WINDOW,
    });
  }
...
function _rollbackCounters(walletKey, repeatKey, chainKey, amountUsdc) {
  // Undo the optimistic counter increments after a failed transfer
  const rl = _refundRateLimit.get(walletKey);
  if (rl && rl.count > 0) rl.count -= 1;

  const rt = _repeatTracker.get(repeatKey);
  if (rt && rt.count > 0) rt.count -= 1;

  _dailySpend[chainKey] = Math.max(
    0,
    (_dailySpend[chainKey] || 0) - amountUsdc,
  );
}
