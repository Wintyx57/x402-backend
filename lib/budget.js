...
class BudgetManager {
    ...
    async loadFromDb() {
        ...
        try {
            const { data, error } = await this.supabase
                .from('budgets')
                .select('wallet, max_usdc, spent_usdc, period, period_start');
    ...
    /**
     * Atomic check + record: prevents race condition where two concurrent requests
     * both pass checkBudget() before either calls recordSpending().
     * The Map mutation is synchronous — DB persist is fire-and-forget.
     * Returns same shape as checkBudget() when denied; on success also includes
     * { alerts, pct, remaining } from the recordSpending logic.
     */
    checkAndRecord(wallet, amountUsdc) {
        const normalized = wallet.toLowerCase();
        const budget = this.budgets.get(normalized);

        // No budget cap set → always allowed, nothing to record
        if (!budget) return { allowed: true, budget: null };

        // Reset period if expired before any arithmetic
        this._resetIfExpired(normalized, budget);
        ...
        if (newTotal > budget.maxUsdc) {
            ...
            logger.warn('Budget', `Blocked: ${normalized.slice(0, 10)}... tried $${amount} but only $${remaining.toFixed(4)} remaining`);
            return {
                allowed: false,
                budget: {
                    ...budget,
                ...
                reason: `Budget exceeded: $${budget.spentUsdc.toFixed(4)} spent of $${budget.maxUsdc.toFixed(2)} limit. Remaining: $${remaining.toFixed(4)}`,
            };
        }

        // Atomically increment in-memory BEFORE async DB persist
        budget.spentUsdc = newTotal;
        ...
        this._persistToDb(normalized, budget); // fire-and-forget
        return { allowed: true, budget, alerts, pct, remaining };
    }

    /** @internal — used only in tests, production uses checkAndRecord() */
