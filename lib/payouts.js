...
function createPayoutManager(supabase) {
  ...
  async function getProviderRevenue(address) {
    // Migration 030 normalized provider_wallet to lowercase — use .eq() with
    // the normalized form so the B-tree index kicks in (was .ilike() = seq scan).
    const { data, error } = await supabase
      .from("pending_payouts")
      .select("service_id, service_name, provider_amount, chain")
      .eq("provider_wallet", (address || "").toLowerCase())
  ...
  async function requestWithdraw(wallet) {
    ...
    const { data, error } = await supabase
      ...
      .eq("provider_wallet", walletLower)
      .eq("status", "pending")
      .order("created_at", { ascending: true });
