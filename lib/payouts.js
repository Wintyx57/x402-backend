...
function createPayoutManager(supabase) {
    ...
    async function getProviderRevenue(address) {
        ...
        if (error) {
            logger.error('Payouts', 'getProviderRevenue error:', { message: error.message });
            return { error: error.message };
        }
    ...
    return { recordPayout, recordSplitPayout, getPendingPayouts, markPayoutsPaid, getRevenueOverview, getProviderRevenue, PLATFORM_FEE_PERCENT };
}
