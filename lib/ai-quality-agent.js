...
const POST_ENDPOINTS = new Set([
    '/api/json-validate', '/api/code', '/api/code-review',
    '/api/contract-risk', '/api/email-parse', '/api/table-insights',
    '/api/detect-lang', '/api/text-stats',
]);
...
async function callApiWithPayment(service, baseUrl) {
    ...
    try {
        ...
        const firstRes = await fetchWithTimeout(callUrl.toString(), firstOptions, API_TIMEOUT);
        ...
        if (isPost) {
            ...
            paidOptions = { method: 'POST', headers: paidHeaders, body: JSON.stringify(testParams) };
        } else {
            paidOptions = { headers: paidHeaders };
        }
        const paidRes = await fetchWithTimeout(callUrl.toString(), paidOptions, API_TIMEOUT);
...
async function evaluateWithGemini(service, responseData, testParams) {
    ...
    try {
        ...
        try {
            ...
            logger.warn('QualityAgent', `JSON parse failed for ${service.name}: raw=${text.slice(0, 300)}`);
            return { _error: `${parseErr.message?.slice(0, 50)} | raw: ${text.slice(0, 200)}` };
        }
        ...
        for (let i = 0; i < sample.length; i++) {
            ...
            if (callResult.error && callResult.error.includes('Payment failed')) {
                ...
                if (consecutivePaymentFailures >= 3) {
                    logger.warn('QualityAgent', `Aborting: ${consecutivePaymentFailures} consecutive payment failures (likely RPC rate limit)`);
                    break;
                }
            } else {
            ...
            const isBinary = typeof callResult.data === 'string'
                && (callResult.data.length > 200 && /^[A-Za-z0-9+/=\s]+$/.test(callResult.data.slice(0, 200)));
            if (isBinary) {
