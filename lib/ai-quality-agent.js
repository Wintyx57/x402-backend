...
async function evaluateWithGemini(service, responseData, testParams) {
    ...
    try {
        ...
        const result = await openaiRetry(async () => {
        ...
        if (typeof parsed.overall_score !== 'number' || !parsed.dimensions) {
            logger.warn('QualityAgent', `Invalid Gemini response structure for ${service.name}`);
            return null;
        }

        return parsed;
    } catch (err) {
        logger.warn('QualityAgent', `Gemini evaluation failed for ${service.name}: ${err.message}`);
        return null;
    }
}
...
async function runQualityAuditOnce() {
    ...
    try {
        ...
        // Process sequentially (nonce ordering)
        for (const service of sample) {
            logger.info('QualityAgent', `Testing ${service.name}...`);
        ...
        sendQualityReport(results, runId, duration, totalSpent, balance).catch(err => {
            logger.warn('QualityAgent', `Telegram report failed: ${err.message}`);
        });
