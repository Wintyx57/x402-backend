...
const MIN_BALANCE = 0.1; // Skip if < 0.10 USDC
const MAX_PRICE_PER_SERVICE = 0.1; // Cap per individual test
...
function initWallet() {
  ...
  logger.info(
    "QualityAgent",
    `Wallet: ${_account.address.slice(0, 10)}... on ${CHAIN_CFG.label}`,
  );
}
...
async function selectSample(supabase, baseUrl) {
  ...
  try {
    ...
    logger.warn(
      "QualityAgent",
      `Failed to load service statuses: ${err.message}`,
    );
  }
  ...
  const scored = candidates.map((svc) => {
    ...
    if (!lastAudit) {
      ...
      const daysSince =
        (now - new Date(lastAudit.checked_at).getTime()) / (86400 * 1000);
...
// POST-only endpoints (from bazaar-discovery _inputSchemaMap)
...
async function callApiWithPayment(service, baseUrl) {
  ...
  try {
    ...
    // Determine if POST endpoint
    const isPost = POST_ENDPOINTS.has(endpointPath);
    ...
    const recipient =
      details.recipient || details.pay_to || process.env.WALLET_ADDRESS;
    ...
    if (isPost) {
      ...
      paidOptions = { headers: paidHeaders };
    }
    const paidRes = await fetchWithTimeout(
      callUrl.toString(),
      paidOptions,
      API_TIMEOUT,
    );
    ...
    try {
      result.data = JSON.parse(paidBody);
    } catch {
      result.data = paidBody;
    }
...
// --- GEMINI EVALUATION ---
const GEMINI_SYSTEM = `You are an API response quality auditor. Respond with ONLY a JSON object, no markdown fences, no explanation.

Score 5 dimensions (0-100):
1. semantic_correctness: logical, plausible values?
2. data_freshness: current data? (score 80 if N/A)
3. locale_accuracy: correct units/formats?
4. content_quality: useful, complete, not placeholder?
5. schema_compliance: matches expected schema?

Respond EXACTLY in this format:
{"overall_score":75,"dimensions":{"semantic_correctness":80,"data_freshness":70,"locale_accuracy":75,"content_quality":80,"schema_compliance":70},"issues":["issue1"],"summary":"Brief summary","severity":"good"}`;
...
async function evaluateWithGemini(service, responseData, testParams) {
  ...
  try {
    ...
    try {
      ...
      logger.warn(
        "QualityAgent",
        `response.text() failed for ${service.name}: ${textErr.message}`,
      );
      return { _error: `text_extract: ${textErr.message?.slice(0, 100)}` };
    }

    // Extract JSON from response — Gemini often wraps in ```json ... ``` or adds text
    let jsonStr = text.trim();
    // Strip markdown code fences (various formats: ```json, ``` json, ```)
    jsonStr = jsonStr
      .replace(/^```\s*(?:json)?\s*\n?/i, "")
      .replace(/\n?```\s*$/i, "")
      .trim();
    // If still not starting with {, try to extract first JSON object
    ...
    try {
      ...
      logger.warn(
        "QualityAgent",
        `JSON parse failed for ${service.name}: raw=${text.slice(0, 300)}`,
      );
      return {
        _error: `${parseErr.message?.slice(0, 50)} | raw: ${text.slice(0, 200)}`,
      };
    }
    ...
    if (typeof parsed.overall_score !== "number" || !parsed.dimensions) {
      logger.warn(
        "QualityAgent",
        `Invalid Gemini structure for ${service.name}: ${jsonStr.slice(0, 200)}`,
      );
    ...
    logger.error(
      "QualityAgent",
      `Gemini eval failed for ${service.name}: ${err.stack || err.message}`,
    );
...
async function runQualityAuditOnce() {
  ...
  logger.info(
    "QualityAgent",
    `=== Starting AI Quality Audit run ${runId.slice(0, 8)} ===`,
  );
  ...
  try {
    ...
    if (balance < MIN_BALANCE) {
      logger.warn(
        "QualityAgent",
        `Balance too low (${balance} < ${MIN_BALANCE}), skipping`,
      );
    ...
    for (let i = 0; i < sample.length; i++) {
      ...
      if (i > 0) await new Promise((r) => setTimeout(r, 5000)); // 5s between calls (RPC rate limits)
      logger.info(
        "QualityAgent",
        `Testing ${service.name} (${i + 1}/${sample.length})...`,
      );
      const callResult = await callApiWithPayment(service, _baseUrl);

      // Abort early if RPC is rate-limiting us (3 consecutive payment failures)
      if (callResult.error && callResult.error.includes("Payment failed")) {
        consecutivePaymentFailures++;
        if (consecutivePaymentFailures >= 3) {
          logger.warn(
            "QualityAgent",
            `Aborting: ${consecutivePaymentFailures} consecutive payment failures (likely RPC rate limit)`,
          );
          break;
        }
      } else {
      ...
      // Evaluate with Gemini (only if we got data, skip binary/image responses)
      let evaluation = null;
      let geminiError = null;
      const isBinary =
        typeof callResult.data === "string" &&
        callResult.data.length > 200 &&
        /^[A-Za-z0-9+/=\s]+$/.test(callResult.data.slice(0, 200));
      if (isBinary) {
        geminiError = "skipped: binary/base64 response";
      } else if (callResult.data && !callResult.error) {
        evaluation = await evaluateWithGemini(
          service,
          callResult.data,
          callResult.test_params,
        );
      ...
      const row = {
        ...
        semantic_correctness:
          evaluation?.dimensions?.semantic_correctness ?? null,
        ...
        error:
          callResult.error || (geminiError ? `gemini: ${geminiError}` : null),
      };
      ...
      if (dbError) {
        logger.warn(
          "QualityAgent",
          `DB insert failed for ${service.name}: ${dbError.message}`,
        );
      }

      results.push(row);

      const scoreStr = evaluation
        ? `score=${evaluation.overall_score}`
        : "no-eval";
      if (callResult.error) {
        logger.warn(
          "QualityAgent",
          `${service.name}: ERROR — ${callResult.error}`,
        );
      } else {
        logger.info(
          "QualityAgent",
          `${service.name}: ${scoreStr} (${callResult.latency_ms}ms)`,
        );
      }
    }
    ...
    sendQualityReport(results, runId, duration, totalSpent, balance).catch(
      (err) => {
        logger.error(
          "QualityAgent",
          `Telegram report failed: ${err.stack || err.message}`,
        );
        // Fallback: send minimal plain text
        notifyAdmin(
          `AI Quality Audit done: ${results.length} APIs, ${scoredCount} scored, ${errCount} errors, spent=${totalSpent.toFixed(4)} USDC`,
        ).catch(() => {});
      },
    );
    ...
    logger.info(
      "QualityAgent",
      `=== Audit complete: ${results.length} APIs, ${totalSpent.toFixed(4)} USDC, ${Math.round(duration / 1000)}s ===`,
    );
    _running = false;
    return {
      status: "success",
...
async function sendQualityReport(
    ...
    for (const r of issues.slice(0, 5)) {
      ...
      const issueText =
        r.issues && r.issues.length > 0
          ? r.issues[0]
          : r.gemini_summary || "Low quality";
      lines.push(
        `${sev.emoji} ${escapeMarkdown(r.service_name)} \u2014 ${escapeMarkdown(String(issueText).slice(0, 60))}, score=${r.overall_score}`,
      );
    }
  }
  ...
  if (coverage > 0) {
    lines.push("");
    lines.push(
      `\uD83D\uDCC8 *Coverage:* ${coverage} APIs audited in last 7 days`,
    );
  }
...
function scheduleNextRun() {
  ...
  logger.info(
    "QualityAgent",
    `Next audit scheduled: ${nextStr} (in ${Math.round(delay / 60000)}min)`,
  );
...
function startQualityAudit(baseUrl, supabase, getGemini) {
  if (!process.env.AGENT_PRIVATE_KEY) {
    logger.warn(
      "QualityAgent",
      "AGENT_PRIVATE_KEY not set — quality audit disabled",
    );
    return;
  }
  if (!getGemini) {
    logger.warn(
      "QualityAgent",
      "Gemini not available — quality audit disabled",
    );
    return;
  }
