...
async function discoverServices(supabase, baseUrl) {
    ...
    for (const svc of data) {
        ...
        // Internal if: owner matches platform wallet OR service URL starts with our base URL
        if (!owner || owner === platformWallet || (normalizedBase && svcUrl.startsWith(normalizedBase))) {
            internal.push(svc);
        } else {
            external.push(svc);
        }
...
// --- TELEGRAM REPORT ---
async function sendTelegramReport(results, runId, durationSeconds, startBalance, endBalance, persistErrors, skippedCount = 0) {
...
async function runDailyTest(baseUrl, supabase) {
    ...
    await sendTelegramReport(results, runId, duration, startBalance, endBalance, persistErrors, aborted ? totalCount : 0);
