...
const { createPublicClient, createWalletClient, http, parseAbi, isAddress } = require('viem');
...
async function testInternalEndpoint(baseUrl, service, supabase, runId) {
    ...
    try {
        ...
        Object.assign(testConfig.params, overrides);
    } catch (err) {
...
// --- TELEGRAM REPORT ---
async function sendTelegramReport(results, runId, durationSeconds, startBalance, endBalance, persistErrors) {
...
async function runDailyTest(baseUrl, supabase) {
    ...
    await sendTelegramReport(results, runId, duration, startBalance, endBalance, persistErrors);

    const pass = results.filter(r => r.overall_status === 'pass').length;
    const partial = results.filter(r => r.overall_status === 'partial').length;
    const fail = results.filter(r => r.overall_status === 'fail').length;
    _lastRunStatus = fail === 0 ? 'success' : 'failed';
    _lastRunResults = { pass, partial, fail, total: results.length, durationSeconds: duration, balanceUsdc: startBalance, endBalanceUsdc: endBalance, persistErrors };
...
function scheduleDailyTest(baseUrl, supabase) {
    ...
    const firstTimer = setTimeout(() => runWithSafetyTimer(), FIRST_RUN_DELAY);
