...
const ENDPOINT_OVERRIDES = {
    ...
    '/api/summarize': { text: 'Artificial intelligence (AI) is intelligence demonstrated by machines, as opposed to the natural intelligence displayed by animals and humans. AI research has been defined as the field of study of intelligent agents, which are systems that perceive their environment and take actions to maximize their chances of success.', maxLength: '100' },
    '/api/sentiment': { text: 'I love this product, it is amazing and works great!' },
    ...
    '/api/text-stats': { text: 'The quick brown fox jumps over the lazy dog. This is a test sentence for text statistics analysis.' },
    '/api/metar': { icao: 'LFPG' },
...
function inferParamValue(key) {
    ...
    if (k.includes('ip') || (k.includes('address') && !k.includes('wallet'))) return '8.8.8.8';
    ...
    if (k.includes('count') || k.includes('limit') || k.includes('max') || k.includes('num')) return '5';
    if (k.includes('page')) return '1';
    if (k.includes('lang') || k.includes('language') || k.includes('locale')) return 'en';
    if (k.includes('format') || k.includes('output')) return 'json';
    if (k.includes('text') || k.includes('content') || k.includes('body') || k.includes('message')) return 'Hello world test';
    if (k.includes('date') || k.includes('start') || k.includes('end')) return '2026-01-15';
    if (k.includes('size') || k.includes('width') || k.includes('height')) return '256';
    if (k.includes('color') || k.includes('hex')) return 'FF6600';
    if (k.includes('token') || k.includes('key') || k.includes('api_key')) return 'test-token-123';
    if (k.includes('wallet') || (k.includes('address') && k.includes('wallet'))) return '0xfb1c478BD5567BdcD39782E0D6D23418bFda2430';
    if (k.includes('lat')) return '48.8566';
    if (k.includes('lon') || k.includes('lng')) return '2.3522';
    if (k.includes('currency') || k.includes('coin') || k.includes('symbol')) return 'USD';
    ...
    if (k.includes('type') || k.includes('kind') || k.includes('category')) return 'general';
    if (k.includes('sort') || k.includes('order')) return 'asc';
    if (k.includes('bool') || k.includes('enable') || k.includes('active')) return 'true';
    return 'test-value';
}
...
function validateResponse(body, expectedFields) {
    ...
    for (const field of expectedFields) {
        if (body[field] !== undefined) {
            ...
            const foundNested = Object.values(body).some(v =>
    ...
    const ratio = expectedFields.length > 0 ? present.length / expectedFields.length : 1;
    ...
    for (let attempt = 0; attempt <= PAID_CALL_RETRIES; attempt++) {
        try {
            ...
            if (result.call_latency_ms > 15000) {
                result.validation_notes = (result.validation_notes ? result.validation_notes + ' | ' : '') +
    ...
    for (let i = 0; i < services.internal.length && !aborted; i += BATCH_SIZE) {
        ...
        if (!aborted && i + BATCH_SIZE < services.internal.length) {
            ...
            if (hasRateLimit) {
                ...
                logger.info('DailyTester', `Rate limit detected — increasing batch delay to ${_dynamicBatchDelay}ms`);
            }
...
module.exports = {
    scheduleDailyTest: scheduleDailyTestWrapped, stopDailyTest, triggerDailyTest, getDailyTesterStatus,
    // Shared utilities (used by ai-quality-agent.js)
    discoverServices, PARAM_DEFAULTS, ENDPOINT_OVERRIDES, inferParamValue, generateParamsFromSchema,
};
