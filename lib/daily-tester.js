...
const BATCH_SIZE = 3; // APIs per batch (reduced to avoid RPC rate limits)
const BATCH_DELAY = 8000; // 8s between batches (public RPCs rate-limit at ~100 tx/30min)
...
const MIN_BALANCE = 1.5; // Skip run if USDC balance < 1.50 (full run costs ~0.90)
const MAX_PRICE_PER_SERVICE = 0.1; // Max USDC per individual test (prevent wallet drain)
...
const ENDPOINT_OVERRIDES = {
  ...
  "/api/avatar": { name: "TestUser", style: "geometric", format: "json" },
  ...
  "/api/code": { language: "javascript", code: "console.log(2 + 2)" },
  "/api/email-parse": {
    email: "From: test@example.com\nSubject: Test\n\nHello world",
  },
  "/api/contract-risk": {
    text: "This agreement limits liability to the amount paid in the previous 12 months. Either party may terminate with 30 days notice.",
  },
  "/api/domain-report": { domain: "google.com" },
  "/api/seo-audit": { url: "https://example.com" },
  "/api/lead-score": { domain: "google.com" },
  // Fix false positives: endpoints that fail with generic "test" params
  "/api/countries": { name: "France" },
  "/api/airquality": { lat: "48.85", lon: "2.35" },
  "/api/qrcode-gen": { text: "https://x402bazaar.org" },
  "/api/base64": { text: "hello world", mode: "encode" },
  "/api/math": { expr: "2+2*3" },
  "/api/unit-convert": { value: "100", from: "km", to: "miles" },
  "/api/cron-parse": { expr: "0 9 * * 1" },
  "/api/summarize": {
    text: "Artificial intelligence (AI) is intelligence demonstrated by machines, as opposed to the natural intelligence displayed by animals and humans. AI research has been defined as the field of study of intelligent agents, which are systems that perceive their environment and take actions to maximize their chances of success.",
    maxLength: "100",
  },
  "/api/sentiment": {
    text: "I love this product, it is amazing and works great!",
  },
  "/api/phone-validate": { phone: "+14155552671" },
  "/api/geocoding": { city: "Paris" },
  // Session 85: fix quality audit 400 errors for newer APIs
  "/api/openverse": { q: "sunset landscape" },
  "/api/itunes": { q: "radiohead", media: "music" },
  "/api/pokemon": { name: "pikachu" },
  "/api/wikipedia": { q: "Bitcoin" },
  "/api/ssl-check": { domain: "google.com" },
  "/api/robots": { url: "https://example.com" },
  "/api/timezone": { lat: "48.85", lon: "2.35" },
  "/api/books": { q: "dune" },
  "/api/crypto": { coin: "bitcoin" },
  "/api/validate-email": { email: "test@example.com" },
  "/api/reddit": { subreddit: "javascript" },
  "/api/malware-check": { url: "https://example.com" },
  "/api/btc-address": { address: "1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa" },
  "/api/url-parse": { url: "https://example.com/path?key=value" },
  "/api/detect-lang": { text: "Bonjour le monde" },
  "/api/text-stats": {
    text: "The quick brown fox jumps over the lazy dog. This is a test sentence for text statistics analysis.",
  },
  "/api/metar": { icao: "LFPG" },
  "/api/opengraph": { url: "https://github.com" },
  "/api/crossref": { query: "machine learning" },
  "/api/iss": {},
...
async function checkPolygonRpc() {
  ...
  try {
    ...
    if (!res.ok) {
      logger.warn(
        "DailyTester",
        `Polygon RPC unhealthy: HTTP ${res.status} (${latencyMs}ms)`,
      );
      return {
        up: false,
        blockNumber: null,
    ...
    if (!blockNumber) {
      logger.warn(
        "DailyTester",
        `Polygon RPC unexpected response: ${JSON.stringify(body)}`,
      );
      return {
        up: false,
        blockNumber: null,
    ...
    logger.info(
      "DailyTester",
      `Polygon RPC OK — block #${blockNumber} (${latencyMs}ms)`,
    );
    ...
    logger.warn(
      "DailyTester",
      `Polygon RPC check failed: ${err.message} (${latencyMs}ms)`,
    );
...
async function checkPolygonFacilitator() {
  ...
  try {
    ...
    logger.info(
      "DailyTester",
      `Polygon facilitator ${up ? "reachable" : "unreachable"}: HTTP ${res.status} (${latencyMs}ms)`,
    );
    return {
      configured: true,
      up,
    ...
    logger.warn(
      "DailyTester",
      `Polygon facilitator check failed: ${err.message} (${latencyMs}ms)`,
    );
    return {
      configured: true,
      up: false,
...
async function updateServiceStatus(supabase, serviceId, overallStatus) {
  ...
  try {
    ...
    logger.warn(
      "DailyTester",
      `Failed to update status for ${serviceId}: ${err.message}`,
    );
  }
}
...
function validatePayment(paymentDetails, _service) {
  ...
  if (
    paymentDetails.recipient === "0x0000000000000000000000000000000000000000"
  ) {
...
let _dynamicBatchDelay = BATCH_DELAY;
...
function initWallet() {
  ...
  if (!pk)
    throw new Error(
      "DAILY_TESTER_KEY / AGENT_PRIVATE_KEY not set — daily tester cannot sign transactions",
    );
  ...
  logger.info(
    "DailyTester",
    `Wallet initialized: ${_account.address.slice(0, 10)}... on ${CHAIN_CFG.label}`,
  );
}
...
async function initNonce() {
  _nonce = await _publicClient.getTransactionCount({
...
async function getUsdcBalance() {
  ...
  const divisor = 10 ** (CHAIN_CFG.usdcDecimals ?? 6);
...
// --- SMART PARAM FALLBACK ---
function inferParamValue(key) {
  if (PARAM_DEFAULTS[key]) return PARAM_DEFAULTS[key];
  const k = key.toLowerCase();
  if (k.includes("url") || k.includes("link")) return "https://example.com";
  if (k.includes("email") || k.includes("mail")) return "test@example.com";
  if (k.includes("domain") || k.includes("host")) return "example.com";
  if (k.includes("ip") || (k.includes("address") && !k.includes("wallet")))
    return "8.8.8.8";
  if (k.includes("name") || k.includes("title")) return "Test";
  if (k.includes("query") || k === "q" || k === "search") return "test query";
  if (k.includes("id") || k.includes("uuid")) return "123";
  if (
    k.includes("count") ||
    k.includes("limit") ||
    k.includes("max") ||
  ...
  if (k.includes("lang") || k.includes("language") || k.includes("locale"))
    return "en";
  if (k.includes("format") || k.includes("output")) return "json";
  if (
    k.includes("text") ||
    k.includes("content") ||
    k.includes("body") ||
  ...
  if (k.includes("date") || k.includes("start") || k.includes("end"))
    return "2026-01-15";
  if (k.includes("size") || k.includes("width") || k.includes("height"))
    return "256";
  if (k.includes("color") || k.includes("hex")) return "FF6600";
  if (k.includes("token") || k.includes("key") || k.includes("api_key"))
    return "test-token-123";
  if (k.includes("wallet") || (k.includes("address") && k.includes("wallet")))
    return "0xfb1c478BD5567BdcD39782E0D6D23418bFda2430";
  if (k.includes("lat")) return "48.8566";
  if (k.includes("lon") || k.includes("lng")) return "2.3522";
  if (k.includes("currency") || k.includes("coin") || k.includes("symbol"))
    return "USD";
  if (k.includes("country")) return "US";
  if (k.includes("phone") || k.includes("tel")) return "+14155551234";
  if (k.includes("password") || k.includes("pass")) return "TestP@ss123!";
  if (k.includes("code")) return "200";
  if (k.includes("type") || k.includes("kind") || k.includes("category"))
    return "general";
  if (k.includes("sort") || k.includes("order")) return "asc";
  if (k.includes("bool") || k.includes("enable") || k.includes("active"))
    return "true";
  return "test-value";
}
...
function generateParamsFromSchema(inputSchema) {
  ...
  for (const key of inputSchema.required) {
    params[key] = inferParamValue(key);
  }
  return params;
}
...
async function discoverServices(supabase, baseUrl) {
  ...
  const renderBase = (process.env.RENDER_EXTERNAL_URL || "")
    .replace(/\/+$/, "")
    .toLowerCase();
  const internal = [];
  const external = [];
  ...
  logger.info(
    "DailyTester",
    `Discovered ${internal.length} internal + ${external.length} external services`,
  );
...
function validateResponse(body, expectedFields) {
  ...
  for (const field of expectedFields) {
    if (body[field] !== undefined) {
      ...
      const foundNested = Object.values(body).some(
        (v) =>
          v &&
          typeof v === "object" &&
          !Array.isArray(v) &&
  ...
  const ratio =
    expectedFields.length > 0 ? present.length / expectedFields.length : 1;
  const valid = missing.length === 0 || (ratio >= 0.7 && missing.length <= 2);
...
async function testInternalEndpoint(baseUrl, service, supabase, runId) {
  ...
  const decimals = CHAIN_CFG.usdcDecimals ?? 6;
  const amountRaw = Math.round(paymentDetails.amount * 10 ** decimals);
  ...
  for (let attempt = 0; attempt <= PAID_CALL_RETRIES; attempt++) {
    try {
      ...
      if (result.call_latency_ms > 15000) {
        result.validation_notes =
          (result.validation_notes ? result.validation_notes + " | " : "") +
          `SLOW: ${(result.call_latency_ms / 1000).toFixed(1)}s`;
      }
      ...
      if (
        ...
        logger.info(
          "DailyTester",
          `${service.name}: HTTP ${res.status}, retrying (${attempt + 1}/${PAID_CALL_RETRIES})...`,
        );
      ...
      if (res.status === 409) {
        ...
        if (
          errBody.includes("TX_ALREADY_USED") ||
          errBody.includes("TX_REPLAY")
        ) {
          result.call_status = "success";
          result.overall_status = "pass";
          result.validation_notes =
            "TX_REPLAY (nonce collision — API is healthy, payment infra issue)";
          logger.warn(
            "DailyTester",
            `${service.name}: TX_REPLAY — marking as pass (API is healthy)`,
          );
          break;
        }
      }
      ...
      if (res.ok) {
        ...
        if (
          ...
          result.response_has_json = false;
          result.validation_notes = `Non-JSON response (${contentType.split(";")[0]}) — 2xx OK`;
          result.overall_status = "pass";
        } else {
...
async function testExternalService(baseUrl, service, supabase, runId) {
  ...
  try {
    ...
    const res402 = await fetchWithTimeout(
      proxyUrl,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    if (res402.status === 400) {
      ...
      if (
        body400 &&
        body400._payment_status === "not_charged" &&
        body400.required_parameters
      ) {
        const retryParams = generateParamsFromSchema(
          body400.required_parameters,
        );
        Object.assign(testParams, retryParams);
        const res402Retry = await fetchWithTimeout(
          proxyUrl,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
        ...
        if (!details) {
          result.payment_error =
            "No payment_details from proxy (after param retry)";
          return result;
        }

        // Continue to payment with these details
        return await completeExternalTest(
          result,
          proxyUrl,
          details,
          testParams,
...
async function completeExternalTest(
  result,
  proxyUrl,
  paymentDetails,
  testParams,
  ...
  for (let attempt = 0; attempt <= PAID_CALL_RETRIES; attempt++) {
    try {
      ...
      if (
        ...
        await res.text().catch(() => ""); // drain stream before retry
        // Proxy uses deferClaim — 502 means payment NOT consumed, safe to retry
        logger.info(
          "DailyTester",
          `${service.name} (external): HTTP ${res.status}, retrying (${attempt + 1}/${PAID_CALL_RETRIES})...`,
        );
        await new Promise((r) => setTimeout(r, 3000 * (attempt + 1)));
        continue;
      }
      ...
      if (res.status === 409) {
        ...
        if (
          ...
          logger.warn(
            "DailyTester",
            `${service.name} (external): TX_REPLAY — marking as pass`,
          );
          break;
        }
      }
...
async function persistResultsSafe(supabase, results) {
  ...
  if (error) {
    logger.warn(
      "DailyTester",
      `Persist failed (table may not exist yet): ${error.message}`,
    );
  }
  return { error };
}
...
async function sendTelegramReport(
  ...
  if (persistErrors > 0)
    lines.push(`\uD83D\uDCBE *DB persist errors:* ${persistErrors}`);
  if (skippedCount > 0)
    lines.push(
      `\u23F9 *Aborted early:* tested ${total}/${skippedCount} (consecutive payment failures)`,
    );
  ...
  if (polygonMetrics) {
    ...
    if (rpc.up) {
      lines.push(
        `  \u2022 RPC: \u2705 block \\#${rpc.blockNumber} \\(${rpc.latencyMs}ms\\)`,
      );
    } else {
      const rpcErr = escapeMarkdown(rpc.error || "unknown");
      lines.push(
        `  \u2022 RPC: \uD83D\uDD34 down \u2014 ${rpcErr.slice(0, 60)}`,
      );
    }
    ...
    if (fac.configured) {
      if (fac.up) {
        lines.push(
          `  \u2022 Facilitator: \u2705 HTTP ${fac.httpStatus} \\(${fac.latencyMs}ms\\)`,
        );
      } else {
        const facErr = escapeMarkdown(
          fac.error || `HTTP ${fac.httpStatus}` || "unreachable",
        );
        lines.push(
          `  \u2022 Facilitator: \u26A0\uFE0F down \u2014 ${facErr.slice(0, 60)}`,
        );
      }
    } else {
  ...
  if (failures.length > 0) {
    ...
    for (const f of failures.slice(0, 15)) {
      const reason = escapeMarkdown(
        f.payment_error || f.call_error || "Unknown",
      );
      lines.push(
        `  \u2022 ${escapeMarkdown(f.label)} \u2014 ${reason.slice(0, 80)}`,
      );
    }
  ...
  if (partials.length > 0) {
    ...
    for (const p of partials.slice(0, 5)) {
      const note = escapeMarkdown(
        p.validation_notes || "Missing expected fields",
      );
      lines.push(
        `  \u2022 ${escapeMarkdown(p.label)} \u2014 ${note.slice(0, 80)}`,
      );
    }
  }
...
async function runDailyTest(baseUrl, supabase) {
  ...
  _dynamicBatchDelay = BATCH_DELAY; // reset rate-limit backoff at start of each run
  ...
  if (!_account) {
    try {
      ...
      await notifyAdmin(
        `\uD83D\uDD34 *Daily Tester Failed*\n\nWallet init error: ${escapeMarkdown(err.message)}`,
      ).catch(() => {});
      return;
    }
  }
  ...
  try {
    ...
    logger.warn(
      "DailyTester",
      `Nonce init failed: ${err.message} — viem will auto-manage nonce`,
    );
  ...
  try {
    ...
    await notifyAdmin(
      `\uD83D\uDD34 *Daily Tester Skipped*\n\nCannot read USDC balance: ${escapeMarkdown(err.message)}`,
    ).catch(() => {});
    return;
  }
  ...
  if (startBalance < MIN_BALANCE) {
    ...
    await notifyAdmin(
      `\u26A0\uFE0F *Daily Tester Skipped*\n\n${msg}\nWallet: \`${_account.address}\`\nChain: ${CHAIN_CFG.label}\n\nEnvoyez du USDC a cette adresse sur SKALE on Base.`,
    ).catch(() => {});
    return;
  }
  ...
  const polygonFacilitator = await checkPolygonFacilitator().catch((err) => {
    logger.warn(
      "DailyTester",
      `Polygon facilitator check threw: ${err.message}`,
    );
    return {
      configured: !!CHAINS.polygon.facilitator,
      up: false,
      httpStatus: null,
      latencyMs: null,
      error: err.message,
  ...
  try {
    ...
    await notifyAdmin(
      `\uD83D\uDD34 *Daily Tester Failed*\n\nCannot load services: ${escapeMarkdown(err.message)}`,
    ).catch(() => {});
    return;
  }
  ...
  logger.info(
    "DailyTester",
    `Discovered ${services.internal.length} internal + ${services.external.length} external = ${totalCount} services`,
  );
  ...
  for (let i = 0; i < services.internal.length && !aborted; i += BATCH_SIZE) {
    ...
    for (const svc of batch) {
      ...
      if (results.length > 0 && results.length % 10 === 0) {
        try {
          ...
          if (midBalance < 0.01) {
            logger.warn(
              "DailyTester",
              `Mid-run abort: balance ${midBalance.toFixed(4)} USDC — too low to continue`,
            );
            aborted = true;
            break;
          }
      ...
      try {
        ...
        logger.info(
          "DailyTester",
          `[${results.length}/${totalCount}] ${svc.name}: ${r.overall_status}`,
        );
        ...
        if (r.payment_status === "failed") {
          ...
          if (consecutivePaymentFailures >= MAX_CONSECUTIVE_PAYMENT_FAILURES) {
            logger.error(
              "DailyTester",
              `Aborting: ${MAX_CONSECUTIVE_PAYMENT_FAILURES} consecutive payment failures — likely wallet/RPC issue`,
            );
            aborted = true;
          }
        } else {
        ...
        if (consecutivePaymentFailures >= MAX_CONSECUTIVE_PAYMENT_FAILURES) {
          logger.error(
            "DailyTester",
            `Aborting: ${MAX_CONSECUTIVE_PAYMENT_FAILURES} consecutive failures`,
          );
          aborted = true;
        }
      }
    ...
    if (results.length > batchStart) {
      const { error: pErr } = await persistResultsSafe(
        supabase,
        results.slice(batchStart),
      );
      if (pErr) persistErrors++;
    }

    // Delay between batches — increase dynamically on rate limits
    if (!aborted && i + BATCH_SIZE < services.internal.length) {
      const batchResults = results.slice(batchStart);
      const hasRateLimit = batchResults.some(
        (r) =>
          r.http_status === 429 ||
          (r.payment_error && r.payment_error.includes("429")),
      );
      if (hasRateLimit) {
        _dynamicBatchDelay = Math.min(_dynamicBatchDelay * 1.5, 15000);
        logger.info(
          "DailyTester",
          `Rate limit detected — increasing batch delay to ${_dynamicBatchDelay}ms`,
        );
      }
      await new Promise((r) => setTimeout(r, _dynamicBatchDelay));
    }
  }
  ...
  if (!aborted && services.external.length > 0) {
    try {
      ...
      logger.info(
        "DailyTester",
        `Nonce re-initialized before external tests: ${_nonce}`,
      );
    } catch (err) {
  ...
  for (const svc of services.external) {
    ...
    try {
      ...
      logger.info(
        "DailyTester",
        `[${results.length}/${totalCount}] ${svc.name} (external): ${r.overall_status}`,
      );
      ...
      if (r.payment_status === "failed") {
        ...
        if (consecutivePaymentFailures >= MAX_CONSECUTIVE_PAYMENT_FAILURES) {
          logger.error(
            "DailyTester",
            `Aborting: ${MAX_CONSECUTIVE_PAYMENT_FAILURES} consecutive payment failures`,
          );
          aborted = true;
        }
      } else {
      ...
      logger.error(
        "DailyTester",
        `${svc.name} (external) crashed: ${err.message}`,
      );
    ...
    const { error: pErr } = await persistResultsSafe(supabase, [
  ...
  let endBalance = null;
  try {
    endBalance = await getUsdcBalance();
  } catch {
    /* non-critical */
  }
  ...
  if (aborted) {
    logger.warn(
      "DailyTester",
      `Run aborted early — tested ${results.length}/${totalCount} services`,
    );
  }
  ...
  _lastRunResults = {
    ...
    balanceUsdc: startBalance,
    endBalanceUsdc: endBalance,
    persistErrors,
    aborted,
  };
  logger.info(
    "DailyTester",
    `Run ${runId} complete: ${pass} pass, ${fail} fail, ${results.length} total in ${duration}s`,
  );
}
...
function scheduleDailyTest(baseUrl, supabase) {
  if (!process.env.DAILY_TESTER_KEY && !process.env.AGENT_PRIVATE_KEY) {
    logger.warn(
      "DailyTester",
      "DAILY_TESTER_KEY / AGENT_PRIVATE_KEY not set — daily tester disabled",
    );
    return;
  }
  ...
  async function runWithSafetyTimer() {
    ...
    const safetyTimer = setTimeout(() => {
      if (_testRunning) {
        logger.error(
          "DailyTester",
          `Safety timer: run exceeded ${SAFETY_TIMER_MS / 60000}min — force-resetting`,
        );
        _testRunning = false;
      }
    }, SAFETY_TIMER_MS);
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        if (lastRunAge < MIN_RUN_INTERVAL) {
          ...
          logger.info(
            "DailyTester",
            `Last run was ${hoursAgo}h ago — skipping startup run (cooldown: ${MIN_RUN_INTERVAL / 3600000}h)`,
          );
          return;
        }
      }
      ...
      logger.warn(
        "DailyTester",
        `Cooldown check failed (${err.message}), running anyway`,
      );
    }
  ...
  try {
    ...
    logger.info(
      "DailyTester",
      `Scheduled: first run in ${FIRST_RUN_DELAY / 1000}s, then every 24h. Wallet: ${_account.address}`,
    );
  } catch (err) {
    logger.error(
      "DailyTester",
      `Wallet init failed at schedule time: ${err.message}`,
    );
  }
}
...
async function triggerDailyTest() {
  if (!_savedBaseUrl || !_savedSupabase) {
    return {
      triggered: false,
      reason: "Daily tester not initialized (ENABLE_DAILY_TESTER not set?)",
    };
  }
  if (_testRunning) {
  ...
  const safetyTimer = setTimeout(() => {
    if (_testRunning) {
      logger.error(
        "DailyTester",
        `Safety timer: run exceeded ${SAFETY_TIMER_MS / 60000}min — force-resetting _testRunning`,
      );
      _testRunning = false;
    }
  }, SAFETY_TIMER_MS);
  ...
  runDailyTest(_savedBaseUrl, _savedSupabase)
    .catch((err) =>
      logger.error("DailyTester", `Manual run failed: ${err.message}`),
    )
    .finally(() => {
      clearTimeout(safetyTimer);
      _testRunning = false;
...
module.exports = {
  scheduleDailyTest: scheduleDailyTestWrapped,
  stopDailyTest,
  triggerDailyTest,
  getDailyTesterStatus,
  // Shared utilities (used by ai-quality-agent.js)
  discoverServices,
  PARAM_DEFAULTS,
  ENDPOINT_OVERRIDES,
  inferParamValue,
  generateParamsFromSchema,
};
