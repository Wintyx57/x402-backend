...
const { notifyAdmin } = require('./telegram-bot');
...
function initWallet() {
    ...
    if (!pk) throw new Error('AGENT_PRIVATE_KEY not set — daily tester cannot sign transactions');
...
async function discoverServices(supabase) {
    const { data, error } = await supabase
        .from('services')
        .select('id, name, url, price_usdc, owner_address');
...
async function testExternalService(baseUrl, service, supabase, runId) {
    ...
    try {
        ...
        // Step 2: Send USDC payment (split mode: 95% to provider, 5% to platform)
        // For daily tester, use simple single payment to recipient (proxy handles verification)
        const amountRaw = Math.round(details.amount * 1e6);
        const payment = await sendUsdcPayment(details.recipient, amountRaw);

        result.payment_status = payment.success ? 'success' : 'failed';
        result.payment_tx_hash = payment.txHash;
        result.payment_amount_usdc = details.amount;
        result.payment_latency_ms = payment.latencyMs;
        result.payment_error = payment.error;
...
async function sendTelegramReport(results, runId, durationSeconds, startBalance) {
    ...
    if (partials.length > 0) {
        ...
        for (const p of partials.slice(0, 5)) {
            const note = p.validation_notes || 'Missing expected fields';
            lines.push(`  \u2022 ${p.label} \u2014 ${note.slice(0, 80)}`);
        }
    }
...
async function runDailyTest(baseUrl, supabase) {
    ...
    try {
        ...
        await notifyAdmin(`\uD83D\uDD34 *Daily Tester Skipped*\n\nCannot read USDC balance: ${err.message}`).catch(() => {});
        return;
    }
    ...
    if (startBalance < MIN_BALANCE) {
        ...
        await notifyAdmin(`\u26A0\uFE0F *Daily Tester Skipped*\n\n${msg}\nWallet: \`${_account.address.slice(0, 10)}...\`\nChain: ${CHAIN_CFG.label}`).catch(() => {});
        return;
    }
    ...
    try {
        ...
        await notifyAdmin(`\uD83D\uDD34 *Daily Tester Failed*\n\nCannot load services: ${err.message}`).catch(() => {});
        return;
    }
...
function scheduleDailyTest(baseUrl, supabase) {
    if (!process.env.AGENT_PRIVATE_KEY) {
        logger.warn('DailyTester', 'AGENT_PRIVATE_KEY not set — daily tester disabled');
        return;
    }
    ...
    logger.info('DailyTester', `Scheduled: first run in ${FIRST_RUN_DELAY / 1000}s, then every 24h`);
}
...
module.exports = { scheduleDailyTest: scheduleDailyTestWrapped, stopDailyTest, triggerDailyTest };
