...
async function openaiRetry(fn, label = 'OpenAI') {
    ...
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
        try {
            ...
            if (attempt < MAX_RETRIES - 1) {
                ...
                logger.warn('OpenAI', `[${label}] OpenAI call failed (status=${status}), retry ${attempt + 1}/${MAX_RETRIES - 1} in ${delayMs / 1000}s`);
                await sleep(delayMs);
            }
        }
    }
    ...
    logger.error('OpenAI', `[${label}] OpenAI call failed after ${MAX_RETRIES} attempts (last status=${status}): ${lastError.message}`);
    throw lastError;
}
