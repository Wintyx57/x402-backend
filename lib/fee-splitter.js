...
const OPERATOR_KEY         = process.env.FEE_SPLITTER_OPERATOR_KEY || process.env.AGENT_PRIVATE_KEY || null;
...
function init() {
    ...
    if (!FEE_SPLITTER_ADDRESS || !OPERATOR_KEY) {
        logger.info('FeeSplitter', 'Not configured — set AGENT_PRIVATE_KEY (or FEE_SPLITTER_OPERATOR_KEY) + POLYGON_FEE_SPLITTER_CONTRACT to enable distribute()');
        return false;
    }
