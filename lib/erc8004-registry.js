...
async function _ensureFeedbackCredits() {
    if (!_feedbackAccount || !_publicClient) {
        return { refilled: false, error: 'feedback wallet or public client not initialized' };
    }
    ...
    try {
        ...
        if (!faucetKey) {
            logger.warn('ERC8004', 'Feedback wallet low on CREDITS but AGENT_PRIVATE_KEY not set — cannot auto-refill');
        ...
        const faucetBalance = await _publicClient.getBalance({ address: faucetAccount.address });
        if (faucetBalance < CREDITS_REFILL_AMOUNT * 2n) {
            logger.warn('ERC8004', `Faucet wallet low: ${(Number(faucetBalance) / 1e18).toFixed(4)} CREDITS — cannot refill feedback wallet`);
        ...
        const faucetWallet = createWalletClient({
            account: faucetAccount, chain, transport,
        });
        ...
        await _publicClient.waitForTransactionReceipt({ hash: txHash, confirmations: 1, timeout: TX_TIMEOUT });

        const newBalance = await _publicClient.getBalance({ address: _feedbackAccount.address });
        logger.info('ERC8004', `Auto-refilled feedback wallet: +2 CREDITS → ${(Number(newBalance) / 1e18).toFixed(4)} CREDITS (tx: ${txHash.slice(0, 18)}...)`);

        return { refilled: true, txHash, balance: (Number(newBalance) / 1e18).toFixed(6) };
    } catch (err) {
...
async function _consolidateOldFaucetWallet() {
    ...
    const normalizedOld = oldFaucetKey.startsWith('0x') ? oldFaucetKey : `0x${oldFaucetKey}`;
    const oldAccount = privateKeyToAccount(normalizedOld);

    // Skip if same wallet (already unified)
    if (oldAccount.address.toLowerCase() === _registryAccount.address.toLowerCase()) return;

    try {
        const balance = await _publicClient.getBalance({ address: oldAccount.address });
        const BUFFER = 10_000_000_000_000_000n; // Keep 0.01 CREDITS for gas of the transfer itself

        if (balance <= BUFFER) {
            logger.info('ERC8004', `Old faucet wallet ${oldAccount.address.slice(0, 10)}... is empty — nothing to consolidate`);
            return;
        }
        ...
        const chain = _buildChain();
        const transport = _buildTransport();
        const oldWallet = createWalletClient({ account: oldAccount, chain, transport });
        ...
        logger.info('ERC8004', `Consolidated ${creditsTransferred} CREDITS from old faucet → agent wallet (tx: ${txHash.slice(0, 18)}...)`);
    } catch (err) {
        logger.warn('ERC8004', `Faucet consolidation failed (non-blocking): ${err.message}`);
    }
}
...
function initClients() {
    ...
    if (feedbackKey) {
        ...
        _ensureFeedbackCredits().then(r => {
            if (r.refilled) logger.info('ERC8004', `Startup refill: +2 CREDITS → ${r.balance}`);
            else if (r.error) logger.warn('ERC8004', `Startup refill skipped: ${r.error}`);
        }).catch(() => {});
...
async function pushAllTrustScores(supabase) {
    ...
    try {
        ...
        if (refillResult.refilled) {
            logger.info('ERC8004', `Feedback wallet auto-refilled before push (balance: ${refillResult.balance})`);
        }
...
async function forcePushAllScores(supabase) {
    ...
    const refillResult = await _ensureFeedbackCredits();
    if (refillResult.refilled) {
        logger.info('ERC8004', `Feedback wallet auto-refilled before force push (balance: ${refillResult.balance})`);
    }
