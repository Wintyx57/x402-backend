...
async function _ensureFeedbackCredits() {
    ...
    try {
        ...
        // Need refill — use FAUCET_PRIVATE_KEY
        const faucetKey = process.env.FAUCET_PRIVATE_KEY;
        if (!faucetKey) {
            logger.warn('ERC8004', 'Feedback wallet low on CREDITS but FAUCET_PRIVATE_KEY not set — cannot auto-refill');
            return { refilled: false, error: 'FAUCET_PRIVATE_KEY not configured' };
        }
