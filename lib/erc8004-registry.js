...
const BATCH_SIZE = 1; // Sequential — avoids nonce conflicts in parallel
const BATCH_DELAY = 500; // ms between TXs (SKALE fast finality)
const TX_TIMEOUT = 30_000; // 30s receipt wait
const BACKEND_URL =
  process.env.RENDER_EXTERNAL_URL || "https://x402-api.onrender.com";
...
// ---------------------------------------------------------------------------
// AUTO-REFILL: send CREDITS from faucet wallet when feedback wallet is low
// ---------------------------------------------------------------------------
const CREDITS_REFILL_THRESHOLD = 2_000_000_000_000_000_000n; // 2.0 CREDITS
const CREDITS_REFILL_AMOUNT = 8_000_000_000_000_000_000n; // 8.0 CREDITS

/**
 * Ensure the feedback wallet has enough CREDITS for gas.
 * If balance < 2.0 CREDITS, sends 8 CREDITS from the agent wallet.
 * @returns {Promise<{refilled: boolean, txHash?: string, balance?: string, error?: string}>}
 */
async function _ensureFeedbackCredits() {
  if (!_feedbackAccount || !_publicClient) {
    return {
      refilled: false,
      error: "feedback wallet or public client not initialized",
    };
  }

  try {
    const balance = await _publicClient.getBalance({
      address: _feedbackAccount.address,
    });

    if (balance >= CREDITS_REFILL_THRESHOLD) {
      return { refilled: false, balance: (Number(balance) / 1e18).toFixed(6) };
    }

    // Need refill — use AGENT_PRIVATE_KEY (unified wallet) or legacy FAUCET_PRIVATE_KEY
    const faucetKey =
      process.env.AGENT_PRIVATE_KEY || process.env.FAUCET_PRIVATE_KEY;
    if (!faucetKey) {
      logger.warn(
        "ERC8004",
        "Feedback wallet low on CREDITS but AGENT_PRIVATE_KEY not set — cannot auto-refill",
      );
      return { refilled: false, error: "AGENT_PRIVATE_KEY not configured" };
    }

    const normalizedKey = faucetKey.startsWith("0x")
      ? faucetKey
      : `0x${faucetKey}`;
    const faucetAccount = privateKeyToAccount(normalizedKey);

    // Check faucet has enough
    const faucetBalance = await _publicClient.getBalance({
      address: faucetAccount.address,
    });
    if (faucetBalance < CREDITS_REFILL_AMOUNT * 2n) {
      logger.warn(
        "ERC8004",
        `Faucet wallet low: ${(Number(faucetBalance) / 1e18).toFixed(4)} CREDITS — cannot refill feedback wallet`,
      );
      return { refilled: false, error: "faucet wallet low on CREDITS" };
    }
    ...
    const txHash = await faucetWallet.sendTransaction({
      to: _feedbackAccount.address,
      value: CREDITS_REFILL_AMOUNT,
      type: "legacy",
    });
    ...
    const newBalance = await _publicClient.getBalance({
      address: _feedbackAccount.address,
    });
    logger.info(
      "ERC8004",
      `Auto-refilled feedback wallet: +2 CREDITS → ${(Number(newBalance) / 1e18).toFixed(4)} CREDITS (tx: ${txHash.slice(0, 18)}...)`,
    );

    return {
      refilled: true,
      txHash,
      balance: (Number(newBalance) / 1e18).toFixed(6),
    };
  } catch (err) {
    logger.error("ERC8004", `Auto-refill failed: ${err.message}`);
    return { refilled: false, error: err.message };
  }
}

/**
 * One-shot: transfer remaining CREDITS from old faucet wallet to the agent wallet.
 * Runs at startup if FAUCET_PRIVATE_KEY is set and differs from AGENT_PRIVATE_KEY.
 * Safe to call multiple times — skips if old faucet balance < 0.1 CREDITS.
 */
async function _consolidateOldFaucetWallet() {
  const oldFaucetKey = process.env.FAUCET_PRIVATE_KEY;
  const agentKey = process.env.AGENT_PRIVATE_KEY;

  // Only consolidate if both keys exist AND are different
  if (!oldFaucetKey || !agentKey) return;
  if (!_registryAccount || !_publicClient) return;

  const normalizedOld = oldFaucetKey.startsWith("0x")
    ? oldFaucetKey
    : `0x${oldFaucetKey}`;
  const oldAccount = privateKeyToAccount(normalizedOld);

  // Skip if same wallet (already unified)
  if (
    oldAccount.address.toLowerCase() === _registryAccount.address.toLowerCase()
  )
    return;
  ...
  try {
    ...
    const BUFFER = 10_000_000_000_000_000n; // Keep 0.01 CREDITS for gas of the transfer itself

    if (balance <= BUFFER) {
      logger.info(
        "ERC8004",
        `Old faucet wallet ${oldAccount.address.slice(0, 10)}... is empty — nothing to consolidate`,
      );
      return;
    }

    const transferAmount = balance - BUFFER;
    const chain = _buildChain();
    const transport = _buildTransport();
    ...
    const txHash = await oldWallet.sendTransaction({
      to: _registryAccount.address,
    ...
    const creditsTransferred = (Number(transferAmount) / 1e18).toFixed(4);
    logger.info(
      "ERC8004",
      `Consolidated ${creditsTransferred} CREDITS from old faucet → agent wallet (tx: ${txHash.slice(0, 18)}...)`,
    );
  } catch (err) {
    logger.warn(
      "ERC8004",
      `Faucet consolidation failed (non-blocking): ${err.message}`,
    );
  }
}
...
function initClients() {
  if (!CHAIN_CFG) {
    logger.warn(
      "ERC8004",
      "SKALE chain config not found — on-chain features disabled",
    );
    return;
  }
  ...
  const registryKey =
    process.env.ERC8004_REGISTRY_KEY || process.env.AGENT_PRIVATE_KEY;
  if (registryKey) {
    ...
    _registryWalletClient = createWalletClient({
      account: _registryAccount,
      chain,
      transport,
    });
    logger.info(
      "ERC8004",
      `Registry wallet: ${_registryAccount.address.slice(0, 10)}...`,
    );
  } else {
    logger.warn(
      "ERC8004",
      "No registry key (AGENT_PRIVATE_KEY) — agent registration disabled",
    );
  }
  ...
  if (feedbackKey) {
    ...
    if (
      _registryAccount &&
      _feedbackAccount.address.toLowerCase() ===
        _registryAccount.address.toLowerCase()
    ) {
      logger.error(
        "ERC8004",
        "CRITICAL: ERC8004_FEEDBACK_KEY = same address as registry wallet! Feedback will be rejected.",
      );
    ...
    _feedbackWalletClient = createWalletClient({
      account: _feedbackAccount,
      chain,
      transport,
    });
    logger.info(
      "ERC8004",
      `Feedback wallet: ${_feedbackAccount.address.slice(0, 10)}...`,
    );

    // Fire-and-forget: refill feedback wallet at startup if low
    _ensureFeedbackCredits()
      .then((r) => {
        if (r.refilled)
          logger.info("ERC8004", `Startup refill: +2 CREDITS → ${r.balance}`);
        else if (r.error)
          logger.warn("ERC8004", `Startup refill skipped: ${r.error}`);
      })
      .catch(() => {});

    // Fire-and-forget: consolidate old faucet wallet CREDITS into agent wallet
    _consolidateOldFaucetWallet().catch(() => {});
  } else {
    logger.info(
      "ERC8004",
      "No ERC8004_FEEDBACK_KEY — on-chain reputation push disabled",
    );
  }
}
...
async function _initNonce(account, label) {
  const nonce = await _publicClient.getTransactionCount({
...
async function registerAgent(
  ...
  if (!_registryWalletClient || !_registryAccount) {
    logger.warn(
      "ERC8004",
      "Registry wallet not configured — skipping on-chain registration",
    );
    return null;
  }
  ...
  try {
    ...
    logger.info(
      "ERC8004",
      `Registered "${serviceName}" → agentId=${agentId} tx=${txHash.slice(0, 18)}...`,
    );
    ...
    if (_registryNonce !== null && _isTimeoutOrNetworkError(err)) {
      logger.warn(
        "ERC8004",
        `Registry TX timeout — re-fetching nonce from chain for "${serviceName}"`,
      );
      try {
        _registryNonce = await _publicClient.getTransactionCount({
          address: _registryAccount.address,
        });
      } catch (nonceErr) {
        logger.warn(
          "ERC8004",
          `Failed to re-fetch registry nonce: ${nonceErr.message}`,
        );
      }
    } else if (
      _registryNonce !== null &&
    ...
    logger.error(
      "ERC8004",
      `Registration failed for "${serviceName}": ${err.message}`,
    );
    return null;
  }
}
...
async function pushTrustScoreFeedback(agentId, trustScore, serviceUrl) {
  ...
  try {
    ...
    if (_feedbackNonce !== null && _isTimeoutOrNetworkError(err)) {
      logger.warn(
        "ERC8004",
        `Feedback TX timeout — re-fetching nonce from chain for agentId=${agentId}`,
      );
      try {
        _feedbackNonce = await _publicClient.getTransactionCount({
          address: _feedbackAccount.address,
        });
      } catch (nonceErr) {
        logger.warn(
          "ERC8004",
          `Failed to re-fetch feedback nonce: ${nonceErr.message}`,
        );
      }
    } else if (
      _feedbackNonce !== null &&
    ...
    logger.warn(
      "ERC8004",
      `Feedback failed agentId=${agentId}: ${err.message}`,
    );
    return null;
  }
}
...
async function pushAllTrustScores(supabase) {
  ...
  if (!_feedbackWalletClient) {
    logger.info(
      "ERC8004",
      "Feedback wallet not configured — skipping reputation push",
    );
  ...
  if (_pushInProgress) {
    logger.info(
      "ERC8004",
      "pushAllTrustScores already in progress — skipping concurrent call",
    );
    return;
  }
  ...
  try {
    // Auto-refill CREDITS if feedback wallet is running low
    const refillResult = await _ensureFeedbackCredits();
    if (refillResult.refilled) {
      logger.info(
        "ERC8004",
        `Feedback wallet auto-refilled before push (balance: ${refillResult.balance})`,
      );
    }
    ...
    if (error || !services || services.length === 0) {
      logger.info(
        "ERC8004",
        "No services with trust_score + erc8004_agent_id — skipping",
      );
      _lastPushResult = {
        ..._lastPushResult,
        ...
        error: error
          ? error.message
          : "No services with trust_score + erc8004_agent_id",
    ...
    logger.info(
      "ERC8004",
      `Reputation push: ${pushed} ok, ${failed} failed / ${services.length} total`,
    );
  } catch (err) {
    _lastPushResult = {
      ..._lastPushResult,
...
async function getFeedbackWalletInfo() {
  ...
  if (_publicClient) {
    try {
      ...
      logger.warn(
        "ERC8004",
        `Failed to read feedback wallet balance: ${err.message}`,
      );
    }
  }
  return {
...
async function forcePushAllScores(supabase) {
  ...
  if (refillResult.refilled) {
    logger.info(
      "ERC8004",
      `Feedback wallet auto-refilled before force push (balance: ${refillResult.balance})`,
    );
  }
  ...
  if (!services || services.length === 0) {
    return {
      ...
      failed: 0,
      total: 0,
      message: "No services with trust_score + erc8004_agent_id",
    };
  }
  ...
  for (let i = 0; i < services.length; i += BATCH_SIZE) {
    ...
    const results = await Promise.all(
      batch.map(async (svc) => {
        try {
          const r = await pushTrustScoreFeedback(
            svc.erc8004_agent_id,
            svc.trust_score,
            svc.url,
          );
  ...
  logger.info(
    "ERC8004",
    `Force push: ${pushed} ok, ${failed} failed / ${services.length} total (${result.duration_ms}ms)`,
  );
  return result;
}
...
async function repairAgentMapping(supabase) {
  ...
  logger.info(
    "ERC8004",
    "Agent mapping lost — restoring from on-chain data...",
  );
  ...
  if (restored > 0) {
    logger.info(
      "ERC8004",
      `Restored ${restored} agent-service mappings from on-chain`,
    );
  }
}
...
module.exports = {
  ...
  forcePushAllScores,
  ensureFeedbackCredits: _ensureFeedbackCredits,
};
