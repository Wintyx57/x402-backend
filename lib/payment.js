...
function createPaymentSystem(supabase, logActivity, budgetManager) {
    ...
    async function verifySplitPayment(txHashProvider, txHashPlatform, totalAmountRaw, chainKey, providerWallet) {
        ...
        const providerResult = await verifyPayment(txHashProvider, providerAmountRaw, chainKey, providerWallet);

        // Platform verification is optional (fallback if absent)
        let platformResult = null;
        if (txHashPlatform) {
            // null recipient → uses WALLET_ADDRESS (platform wallet)
            platformResult = await verifyPayment(txHashPlatform, platformAmountRaw, chainKey, null);
        }
