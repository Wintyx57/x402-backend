...
function createPaymentSystem(supabase, logActivity, budgetManager) {
    ...
    function paymentMiddleware(minAmountRaw, displayAmount, displayLabel, options = {}) {
        ...
        return async (req, res, next) => {
            ...
            if (!txHash) {
                ...
                const responseBody = {
                    ...
                    payment_details: {
                        ...
                        // Phase 2 Polygon facilitator: recipient is the FeeSplitter contract.
                        // Without this, the MCP would send USDC directly to WALLET_ADDRESS
                        // instead of the FeeSplitter, causing facilitator verification to fail.
                        recipient: (() => {
                            const chain = getChainConfig(chainKey);
            ...
            // Verification: use facilitator (Phase 2) for Polygon when configured,
            // otherwise fall back to direct on-chain RPC verification (Phase 1).
            const _verifyChain = getChainConfig(chainKey);
            const _useFacilitator = chainKey === 'polygon' && !!(_verifyChain && _verifyChain.facilitator);
            try {
                const result = _useFacilitator
                    ? await verifyViaFacilitator(txHash, minAmountRaw, chainKey)
                    : await verifyPayment(txHash, minAmountRaw, chainKey);
                ...
                const isNetworkError = err.message === 'RPC timeout'
                    ...
                    || err.message.includes('facilitator');
                if (isNetworkError) {
                    const errorSource = _useFacilitator ? 'Polygon facilitator' : 'RPC node';
                    return res.status(503).json({
