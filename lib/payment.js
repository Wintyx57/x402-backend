...
function createPaymentSystem(supabase, logActivity, budgetManager) {
        ...
        for (const log of receipt.logs) {
            if (log.topics[0] === TRANSFER_TOPIC && log.topics.length >= 3) {
                ...
                if (toAddress === serverAddress) {
                    ...
                    // SKALE on Base USDC has 18 decimals (bridge-wrapped); Base and Polygon use 6 decimals.
                    const usdcDivisor = chainKey === 'skale' ? 1e18 : 1e6;
                    if (amount >= BigInt(minAmount)) {
                        ...
                        logger.info('x402', `Insufficient amount on ${chain.label}: ${Number(amount) / usdcDivisor} USDC (min: ${Number(minAmount) / usdcDivisor})`);
                    }
                }
            }
        }
    ...
    function paymentMiddleware(minAmountRaw, displayAmount, displayLabel, options = {}) {
        ...
        return async (req, res, next) => {
            ...
            if (!txHash) {
                ...
                const responseBody = {
                    ...
                    payment_details: {
                        ...
                        // Default recipient: platform wallet. For proxied services with
                        // owner_address, proxy.js overrides this to FeeSplitter contract.
            ...
            // For direct middleware calls (native wrappers): verify transfer to WALLET_ADDRESS.
            // The proxy overrides recipient to FeeSplitter for external services.
            try {
                const result = await verifyPayment(txHash, minAmountRaw, chainKey, null);
