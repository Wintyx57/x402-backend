...
function fetchWithTimeout(url, options, timeout = RPC_TIMEOUT, signal) {
    ...
    // Merge external abort signal into fetch options
    const fetchOpts = signal ? { ...options, signal } : options;
    return Promise.race([
        fetch(url, fetchOpts).finally(() => clearTimeout(timerId)),
        timeoutPromise,
    ]);
}
...
async function fetchWithFallback(chain, body, timeout, signal) {
    ...
    for (const url of urls) {
        try {
            return await fetchWithTimeout(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }, timeout, signal);
        } catch (err) {
            lastError = err;
            continue;
        }
...
function createPaymentSystem(supabase, logActivity, budgetManager) {
    ...
    async function verifyPayment(txHash, minAmount, chainKey = DEFAULT_CHAIN_KEY, recipientAddress = null) {
        ...
        const globalTimer = setTimeout(() => abortController.abort(), VERIFY_TIMEOUT_MS);
        try {
            ...
            const requiredConfirmations = chainKey === 'skale' ? 0 : (chainKey === 'polygon' ? 5 : 2);
            ...
            for (let attempt = 0; attempt < maxRetries; attempt++) {
                ...
                if (!receipt) {
                    ...
                    }, RPC_TIMEOUT, abortController.signal);
                ...
                if (!statusOk) {
                    if (attempt < maxRetries - 1) {
                        logger.info('x402', `Tx ${normalizedTxHash.slice(0, 18)}... on ${chain.label}: not found yet, waiting ${retryDelayMs}ms (attempt ${attempt + 1}/${maxRetries})`);
                ...
                if (attempt < maxRetries - 1) {
                    logger.info('x402', `Tx ${normalizedTxHash.slice(0, 18)}... on ${chain.label}: ${confirmations} confirmation(s), waiting ${retryDelayMs}ms (attempt ${attempt + 1}/${maxRetries})`);
                    await new Promise(r => setTimeout(r, retryDelayMs));
                }
            }
        ...
        return async (req, res, next) => {
            ...
            try {
                ...
                if (!deferClaim) {
                    ...
                    req._releasePaymentLock = () => _pendingClaims.delete(replayKey);
                }
            }
