...
// Internal proxy bypass: allows the proxy to call internal services without double-payment.
// Derived deterministically from ADMIN_TOKEN so all instances share the same secret.
const INTERNAL_PROXY_SECRET = crypto.createHash('sha256')
    .update(`x402-internal-proxy:${process.env.ADMIN_TOKEN || process.env.SUPABASE_KEY || 'dev'}`)
    .digest('hex');
...
function createPaymentSystem(supabase, logActivity, budgetManager) {
    ...
    async function verifyPayment(txHash, minAmount, chainKey = DEFAULT_CHAIN_KEY, recipientAddress = null) {
        ...
        // 1. Fetch receipt + wait for 2 confirmations (retry up to 10s for fresh txs)
        let receipt = null;
        let confirmations = 0;
    ...
    function paymentMiddleware(minAmountRaw, displayAmount, displayLabel) {
        return async (req, res, next) => {
            // Internal proxy bypass: the proxy already verified payment on-chain
            // before calling internal services. Skip double-payment verification.
            if (req.headers['x-internal-proxy'] === INTERNAL_PROXY_SECRET) {
                return next();
            }
    ...
    return { paymentMiddleware, verifyPayment, verifySplitPayment, fetchWithTimeout };
}
