...
function createPaymentSystem(supabase, logActivity, budgetManager) {
    ...
    async function verifyPayment(txHash, minAmount, chainKey = DEFAULT_CHAIN_KEY) {
        ...
        if (!receipt || receipt.status !== '0x1') {
            logger.info('x402', `Tx ${normalizedTxHash.slice(0, 18)}... on ${chain.label}: failed or not found`);
            return false;
        }

        // 1b. Verifier le nombre de confirmations (minimum 2 blocs)
        // If transaction is very fresh, wait up to 8 seconds for confirmations
        ...
        for (let attempt = 0; attempt < 3; attempt++) {
            ...
            if (attempt < 2) {
                logger.info('x402', `Tx ${normalizedTxHash.slice(0, 18)}... on ${chain.label}: ${confirmations} confirmation(s), waiting 4s (attempt ${attempt + 1}/3)`);
                await new Promise(r => setTimeout(r, 4000));
            }
        }
        if (confirmations < 2) {
            logger.info('x402', `Tx ${normalizedTxHash.slice(0, 18)}... on ${chain.label}: only ${confirmations} confirmation(s) after retries, minimum 2 required`);
            return false;
        }
        ...
        return async (req, res, next) => {
            ...
            try {
                ...
                if (result && result.valid) {
                    ...
                    if (agentWallet && budgetManager) {
                        const budgetResult = budgetManager.recordSpending(agentWallet, displayAmount);
                        if (budgetResult) {
