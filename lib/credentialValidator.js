...
async function validateCredentials(upstreamUrl, rawCredentials, options = {}) {
  ...
  if (
    !rawCredentials ||
    !rawCredentials.type ||
    !rawCredentials.credentials?.length
  ) {
    return { valid: true };
  }
  ...
  const { headers: injectedHeaders, url: injectedUrl } = injectCredentials(
    headers,
    upstreamUrl,
    rawCredentials,
  );
  ...
  if (!options.skipSsrf) {
    try {
      ...
      return {
        valid: false,
        error: `URL blocked by security policy: ${err.message}`,
      };
    }
  }
  ...
  const result = await pingUpstream(
    injectedUrl,
    injectedHeaders,
    "HEAD",
    timeoutMs,
  ...
  if (result.status === 405 || result.status === 402) {
    ...
    const getResult = await pingUpstream(
      injectedUrl,
      injectedHeaders,
      "GET",
      timeoutMs,
    ...
    if (getInterpreted.is402 && getResult.headers402) {
      const normalized = normalize402(
        402,
        getResult.headers402,
        getResult.body402 || {},
      );
...
function interpretResponse(result) {
  ...
  if (status === 402) {
    return {
      valid: true,
      warning: "Upstream requires payment (HTTP 402) — this is a paid API",
      is402: true,
    };
  }
