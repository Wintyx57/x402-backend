...
const STOPWORDS = new Set([
    'i', 'need', 'want', 'to', 'a', 'an', 'the', 'for', 'of', 'and', 'or',
    'in', 'on', 'with', 'that', 'this', 'get', 'find', 'me', 'my', 'some',
    'can', 'you', 'do', 'is', 'it', 'be', 'have', 'use', 'please', 'should',
    'would', 'could', 'how', 'what', 'where', 'which',
]);
...
async function fetchCandidates(supabase, keywords, originalQuery) {
    ...
    // Query 1: OR across all keywords in name + description
...
function scoreService(service, keywords, originalQuery) {
    ...
    const tags      = Array.isArray(service.tags) ? service.tags.map(t => t.toLowerCase()) : [];
...
async function getServicesWithCache(supabase) {
    ...
    if (_servicesListCache && (now - _servicesListCache.fetchedAt) < SERVICES_CACHE_TTL_MS) {
    ...
    if (error) {
        logger.warn('SmartSearch', `Could not refresh services cache: ${error.message}`);
...
async function askGemini(getGemini, query, serviceNames) {
    ...
    try {
        ...
        logger.warn('SmartSearch', `Gemini JSON parse error: ${parseErr.message} — raw="${rawText.slice(0, 200)}"`);
    }

    _geminiCache.set(cacheKey, { suggestions, fetchedAt: now });
    logger.info('SmartSearch', `Gemini suggestions cached — query="${cacheKey}" count=${suggestions.length}`);
...
async function smartSearch(supabase, query, getGemini = null) {
    ...
    logger.info('SmartSearch', `query="${trimmed}" keywords=[${keywords.join(', ')}]`);
    ...
    if (topScore >= SCORE_THRESHOLD) {
        logger.info('SmartSearch', `method=scoring results=${ranked.length} topScore=${topScore}`);
        return {
    ...
    if (!getGemini) {
        ...
        logger.info('SmartSearch', `method=scoring (no Gemini) results=${ranked.length} topScore=${topScore}`);
        return {
            results: enrichWithParams(ranked),
            query_used: trimmed,
            keywords_used: keywords,
    ...
    logger.info('SmartSearch', `topScore=${topScore} < ${SCORE_THRESHOLD} — invoking Gemini fallback`);
    ...
    try {
        ...
        logger.info('SmartSearch', `method=scoring+gemini results=${merged.length} gemini=${geminiRanked.length}`);
