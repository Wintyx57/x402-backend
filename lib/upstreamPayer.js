...
function _getClients(chainKey) {
  ...
  // Use viem's fallback() transport across every configured RPC so a primary
  // RPC outage doesn't take the relay offline. Previously this function used
  // only chainCfg.rpcUrl (scalar), which silently failed over to nothing.
  const urls =
    Array.isArray(chainCfg.rpcUrls) && chainCfg.rpcUrls.length > 0
      ? chainCfg.rpcUrls.filter(Boolean)
      : [chainCfg.rpcUrl].filter(Boolean);
  let transport;
  if (urls.length > 1) {
    // Import lazily to avoid adding a top-level dep on `fallback` if unused.
 ...
 * @returns {Promise<{ success, xPaymentV1, paymentSignatureV2, signature, authorization, chain, amount, error? }>}
 */
async function signEIP3009ForUpstream(normalized) {
  ...
  try {
    ...
    // Build BOTH v1 and v2 X-PAYMENT headers for maximum compatibility

    // v1 format (X-PAYMENT header)
    // Map internal chain keys to x402 standard network names (Coinbase spec)
    ...
    const v1Payload = {
      x402Version: 1,
      scheme: "exact",
      network: CHAIN_TO_X402_NETWORK[chainKey] || chainKey,
      payload: { signature, authorization },
    };
    const xPaymentV1 = Buffer.from(JSON.stringify(v1Payload)).toString(
      "base64",
    );

    // v2 format (PAYMENT-SIGNATURE header) — includes resource + accepted from the 402 response
    const CHAIN_TO_CAIP2 = {
      base: "eip155:8453",
      polygon: "eip155:137",
    ...
    const v2Payload = {
      x402Version: 2,
      resource: {
        url: normalized.resource || recipient,
        description: normalized.description || "",
        mimeType: "application/json",
      },
      accepted: {
        scheme: normalized.scheme || "exact",
        network: CHAIN_TO_CAIP2[chainKey] || chainKey,
        amount: amount.toString(),
        asset: normalized.asset || domain.verifyingContract,
        payTo: recipient,
        maxTimeoutSeconds: normalized.maxTimeoutSeconds || 60,
        extra: normalized.extra || { name: "USD Coin", version: "2" },
      },
      payload: { signature, authorization },
    };
    const paymentSignatureV2 = Buffer.from(JSON.stringify(v2Payload)).toString(
      "base64",
    );

    logger.info(
      "UpstreamPayer",
      `Signed EIP-3009 for ${Number(amount) / 1e6} USDC to ${recipient.slice(0, 10)}... on ${chainKey} (v1+v2)`,
    );
...
async function getRelayBalance(chainKey) {
  ...
  try {
    ...
    // A silent null here disabled the Telegram low-balance alert with no trace.
    // Log so operators can investigate when the alert suddenly stops firing.
    logger.error(
      "UpstreamPayer",
      `getRelayBalance(${chainKey}) failed: ${err?.message || err}`,
    );
    return null;
  }
}
