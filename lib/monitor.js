...
async function fetchExternalServices(supabase) {
  ...
  try {
    ...
    // Skip quarantined and still-validating services — health-checking them
    // just generates noisy state transitions and wastes RPC/http budget.
...
async function updateServicesStatus(supabase, results) {
  ...
  try {
    ...
    // SKIP quarantined services — quarantine is sticky (only admin can remove)
    const byStatus = {};
