...
async function fetchExternalServices(supabase) {
  ...
  try {
    ...
    const { data, error } = await supabase
      .from('services')
      .select('id, name, url, owner_address, encrypted_credentials, credential_type')
      .limit(500);
...
async function checkAllEndpoints(baseUrl) {
  ...
  const batchResults = await Promise.all(
    batches.map(batch => Promise.all(batch.map(ep => checkEndpoint(baseUrl, ep))))
  );
...
async function checkExternalEndpoints(supabase) {
  ...
  const batchResults = await Promise.all(
    batches.map(batch => Promise.all(batch.map(ep => checkExternalEndpoint(ep))))
  );
