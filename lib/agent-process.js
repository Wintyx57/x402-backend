...
function startAgent() {
    ...
    // Clean up any leftover .env file on disk (legacy behavior — secrets must NOT be written to disk)
    const envFilePath = path.join(AGENT_DIR, '.env');
    if (fs.existsSync(envFilePath)) {
        try {
            fs.unlinkSync(envFilePath);
            logger.info('CommunityAgent', 'Removed legacy .env file from disk');
        } catch (unlinkErr) {
            logger.warn('CommunityAgent', `Could not remove legacy .env file: ${unlinkErr.message}`);
        }
    }

    // Build child environment: inherit parent env and add/override forwarded vars
    const FORWARD_VARS = [
    ...
    logger.info('CommunityAgent', `Spawning agent with env vars — ${statusParts.join(', ')}`);
