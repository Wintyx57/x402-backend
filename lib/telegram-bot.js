...
// Commands: /balance, /stats, /status, /recent, /services, /uptime, /top, /revenue, /search, /endpoint, /verif, /help
...
function sanitizePostgrest(input) {
    ...
    const operatorPattern = /\.(ilike|eq|neq|not|in|cs|cd|is|gt|gte|lt|lte|like|fts)\./i;
...
async function sendMessage(token, chatId, text, parseMode = 'Markdown') {
    try {
        ...
        if (!res.ok && parseMode === 'Markdown') {
            ...
            if (errBody.description && errBody.description.includes("can't parse")) {
                logger.warn('TelegramBot', `Markdown parse failed, retrying plain text`);
                ...
                logger.error('TelegramBot', `sendMessage API error: ${errBody.description || res.status}`);
            }
        }
    } catch (err) {
...
async function handleServices(token, chatId, supabase) {
    ...
    for (const s of services) {
        ...
        lines.push(`\u2022 *${escapeMarkdown(s.name)}* — ${price}${tags}`);
    }
...
async function handleSearch(token, chatId, supabase, query) {
    ...
    const q = sanitizePostgrest(query);
    ...
    try {
        ...
        for (const s of data) {
            const price = s.price_usdc > 0 ? `$${Number(s.price_usdc).toFixed(3)}` : 'FREE';
            const desc = escapeMarkdown((s.description || '').slice(0, 60));
            const tags = s.tags && s.tags.length > 0 ? `\n   Tags: ${s.tags.slice(0, 4).join(', ')}` : '';
            lines.push(`\u2022 *${escapeMarkdown(s.name)}* — ${price}`);
...
async function handleEndpoint(token, chatId, supabase, getMonitorStatus, query) {
    ...
    try {
        ...
        const lines = [
            ...
            ``,
            `*Description:* ${escapeMarkdown((s.description || 'N/A').slice(0, 100))}`,
            `*Prix:* ${price} USDC`,
            `*URL:* \`${s.url || 'N/A'}\``,
            `*Tags:* ${tags}`,
...
// --- Command: /help ---
// --- /verif — Trigger daily E2E test manually ---
async function handleVerif(token, chatId) {
    // Lazy require to avoid circular dependency (daily-tester requires telegram-bot)
    const { triggerDailyTest } = require('./daily-tester');
    const result = await triggerDailyTest();
    if (result.triggered) {
        return sendMessage(token, chatId, '\u2705 *Daily E2E test lance !*\n\nLe rapport sera envoye ici dans ~10 minutes.');
    } else {
        return sendMessage(token, chatId, `\u26A0\uFE0F *Impossible de lancer le test*\n\n${result.reason}`);
    }
}

async function handleHelp(token, chatId) {
    const text = [
        `*x402 Bazaar Bot* \u2014 12 commandes disponibles:`,
        ``,
        `\uD83D\uDCCA *Tableau de bord*`,
        ...
        ``,
        `\uD83D\uDD0D *Monitoring*`,
        `/status \u2014 Status des 95 endpoints`,
        `/uptime \u2014 Uptime (24h/7d/30d)`,
        `/verif \u2014 Lancer le test E2E de toutes les APIs`,
        ``,
        `\uD83D\uDCE6 *Services*`,
...
async function processMessage(token, chatId, message, supabase, getMonitorStatus) {
    ...
    switch (command) {
        ...
        case '/verif':
            return handleVerif(token, chatId);
        case '/help':
        case '/start':
...
module.exports = { startTelegramBot, stopTelegramBot, notifyAdmin, escapeMarkdown };
