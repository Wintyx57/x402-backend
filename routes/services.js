...
function createServicesRouter(
  ...
  const BASE_COLUMNS =
    "id, name, url, price_usdc, description, owner_address, tags, verified_status, verified_at, created_at, required_parameters, status, last_checked_at, trust_score, trust_score_updated_at, erc8004_agent_id, erc8004_registered_at, credential_type, encrypted_credentials";
  ...
  supabase
    ...
    .then(({ error }) => {
      if (!error) {
        ...
        logger.info(
          "Services",
          "logo_url column detected — included in queries",
        );
      }
    })
    .catch(() => {});
  ...
  // --- LISTE DES SERVICES (GRATUIT) ---
  router.get("/services", paidEndpointLimiter, async (req, res) => {
  ...
  // --- RECHERCHE DE SERVICES (GRATUIT) — Smart Search (scoring + Gemini fallback) ---
  router.get("/search", paidEndpointLimiter, async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        /* Gemini not configured — scoring only */
      }
      ...
      logActivity(
        "search",
        `Search "${query}" -> ${searchResult.results.length} result(s) [${searchResult.method}]`,
      );
  ...
  router.get("/api/health-check", dashboardApiLimiter, async (req, res) => {
    try {
      ...
      const urls = [
        ...new Set((services || []).map((s) => s.url).filter(Boolean)),
      ];
    ...
    router.post(
      ...
      async (req, res) => {
        ...
        const { data: services, error: fetchErr } = await supabase
          .from("services")
          .select(
            "id, name, url, price_usdc, verified_status, owner_address, required_parameters",
          )
          .eq("id", id)
          .limit(1);
        ...
        logActivity(
          "admin",
          `Re-verified "${service.name}": ${report.verdict}`,
        );
        logger.info(
          "Admin",
          `Re-verified "${service.name}" (${id.slice(0, 8)}): ${report.verdict}`,
        );
  ...
  if (adminAuth) {
    router.delete("/api/admin/services/:id", adminAuth, async (req, res) => {
      ...
      if (error) {
        logger.error(
          "Admin",
          `DELETE /api/admin/services/${id} error: ${error.message}`,
        );
    ...
    router.get("/api/admin/debug-verify", adminAuth, async (req, res) => {
      ...
      if (!TX_HASH_REGEX.test(txHash)) {
        return res.status(400).json({
          error:
            "Invalid tx hash format — expected 0x followed by 64 hex characters",
        });
      }
      ...
      try {
        ...
        steps.push({
          ...
          length: normalizedTxHash.length,
        });
        ...
        steps.push({
          ...
          env_set: !!process.env.WALLET_ADDRESS,
        });
        ...
        steps.push({
          ...
          chainKey,
          label: chain.label,
          rpcCount: (chain.rpcUrls || [chain.rpcUrl]).filter(Boolean).length,
        ...
        if (!receipt) {
          steps.push({ step: "FAIL", reason: "No receipt found" });
      ...
      const { error } = await supabase.from("discovered_apis").upsert(
        {
          ...
          agent_address: agent_address
            ? String(agent_address).slice(0, 100)
            : null,
