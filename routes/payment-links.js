...
const PUBLIC_FIELDS =
  "id, title, description, price_usdc, owner_address, is_active, views, paid_count, total_earned_usdc, redirect_after_payment, created_at, updated_at";
...
function buildPaymentRequired(link, chainKey = DEFAULT_CHAIN_KEY) {
  ...
  const recipient =
    chainCfg && chainCfg.feeSplitterContract
  ...
  return {
    ...
    instructions:
      "Send USDC payment, then POST to /api/payment-links/:id/access with X-Payment-TxHash and X-Payment-Chain headers.",
  };
}
 ...
 */
function createPaymentLinksRouter(
  supabase,
  logActivity,
  createLinkLimiter,
  paymentSystem,
  ...
  router.post("/api/payment-links", createLinkLimiter, async (req, res) => {
    ...
    if (!parsed.success) {
      return res.status(400).json({
        ...
        details: (parsed.error.issues || parsed.error.errors || []).map(
          (e) => ({ field: e.path.join("."), message: e.message }),
        ),
      });
    }
    ...
    const sigCheck = await verifyEip191Signature(
      message,
      ownerAddress,
      signature,
      timestamp,
    ...
    if (!sigCheck.valid) {
      return res
        .status(401)
        .json({ error: "Invalid signature", reason: sigCheck.reason });
    }
    ...
    const { data: link, error: insertErr } = await supabase
      ...
      .select(
        "id, title, description, price_usdc, owner_address, is_active, redirect_after_payment, created_at",
      )
      .single();
    ...
    notifyAdmin(
      `New payment link created: "${title}" @ ${priceUsdc} USDC by ${ownerAddress.slice(0, 10)}...`,
    ).catch(() => {});
    logActivity(
      "register",
      `Payment link created: "${title}" @ ${priceUsdc} USDC`,
    );
  ...
  router.get("/api/payment-links/my/:address", async (req, res) => {
    ...
    if (error) {
      logger.error(
        "PaymentLinks",
        `List error for ${address}: ${error.message}`,
      );
  ...
  router.get("/api/payment-links/:id", async (req, res) => {
    ...
    if (!link.is_active) {
      return res
        .status(410)
        .json({ error: "Payment link has been deactivated" });
    }
  ...
  router.post("/api/payment-links/:id/access", async (req, res) => {
    ...
    const { data: link, error: fetchErr } = await supabase
      .from("payment_links")
      .select(
        "id, title, description, price_usdc, owner_address, is_active, redirect_after_payment, paid_count, total_earned_usdc, target_url",
      )
      .eq("id", id)
      .single();
    ...
    try {
      ...
      if (existing && existing.length > 0) {
        return res.status(409).json({
          ...
          code: "TX_REPLAY",
          message:
            "This transaction hash has already been used. Please send a new transaction.",
        });
      }
    } catch (err) {
    ...
    try {
      ...
      logger.error(
        "PaymentLinks",
        `Verification error on ${chainKey}: ${err.message}`,
      );
      ...
      if (isNetworkError) {
        return res
          ...
          .json({
            ...
            message: "RPC node unreachable. Please retry.",
          });
      }
      return res
        .status(402)
    ...
    const claimed = await markTxUsed(
      replayKey,
      `Payment Link Access: ${link.title}`,
    );
    if (!claimed) {
      return res.status(409).json({
        error: "TX_ALREADY_USED",
        code: "TX_REPLAY",
        message:
          "This transaction hash has already been claimed by another request.",
      });
    }
    ...
    if (payoutManager && link.owner_address) {
      payoutManager
        ...
        .catch((err) => {
          logger.error(
            "PaymentLinks",
            `Failed to record payout for "${link.title}": ${err.message}`,
          );
        });
    }
    ...
    supabase
      ...
      .update({
        ...
        total_earned_usdc: Number(
          (Number(link.total_earned_usdc || 0) + price).toFixed(4),
        ),
      })
      .eq("id", id)
      .then(() => {})
    ...
    logActivity(
      "payment_link",
      `Payment link accessed: "${link.title}" @ ${price} USDC (95/5 split)`,
      price,
      txHash,
    );
  ...
  router.delete("/api/payment-links/:id", async (req, res) => {
    ...
    if (!ownerAddress || !signature || !timestamp) {
      return res
        .status(400)
        .json({
          error: "Missing required fields: ownerAddress, signature, timestamp",
        });
    }
    ...
    if (link.owner_address.toLowerCase() !== ownerAddress.toLowerCase()) {
      return res
        ...
        .json({
          error: "Forbidden",
          message: "You are not the owner of this payment link.",
        });
    }
    ...
    if (!link.is_active) {
      return res
        .status(410)
        .json({ error: "Payment link is already deactivated" });
    }
    ...
    if (updateErr) {
      ...
      return res
        .status(500)
        .json({ error: "Failed to deactivate payment link" });
    }
