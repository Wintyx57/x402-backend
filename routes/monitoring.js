...
function createMonitoringRouter(supabase, statsLimiter) {
  ...
  router.get("/api/status/uptime", statsLimiter, async (req, res) => {
    ...
    if (!ms) {
      return res
        .status(400)
        .json({ error: "Invalid period. Use 24h, 7d, or 30d." });
    }
    ...
    try {
      ...
      const totalOnline = uptime.reduce(
        (sum, u) => sum + (u.uptime !== null ? (u.uptime / 100) * u.checks : 0),
        0,
      );
      const overallUptime =
        totalChecks > 0
          ? parseFloat(((totalOnline / totalChecks) * 100).toFixed(2))
          : null;
  ...
  router.get("/api/public-stats", statsLimiter, async (req, res) => {
    ...
    const servicesCount =
      servicesCountResult.status === "fulfilled"
        ...
        : 0;
    const totalApiCalls =
      apiCallsCountResult.status === "fulfilled"
        ? apiCallsCountResult.value.count || 0
        : 0;
    const totalPayments =
      paymentsCountResult.status === "fulfilled"
        ? paymentsCountResult.value.count || 0
        : 0;
    const recentCallCount24h =
      recentCallsResult.status === "fulfilled"
        ? recentCallsResult.value.count || 0
        : 0;
    ...
    if (apiCallsCountResult.status === "rejected")
      logger.warn(
        "PublicStats",
        `Failed to count API calls: ${apiCallsCountResult.reason?.message}`,
      );
    if (paymentsCountResult.status === "rejected")
      logger.warn(
        "PublicStats",
        `Failed to count payments: ${paymentsCountResult.reason?.message}`,
      );
    if (recentCallsResult.status === "rejected")
      logger.warn(
        "PublicStats",
        `Failed to count 24h calls: ${recentCallsResult.reason?.message}`,
      );
    ...
    if (topCallsResult.status === "fulfilled" && topCallsResult.value.data) {
      ...
      logger.warn(
        "PublicStats",
        `Failed to compute top endpoints: ${topCallsResult.reason?.message}`,
      );
    }
    ...
    if (
      ...
      uptimeResult.value.data?.length > 0
    ) {
      ...
      logger.warn(
        "PublicStats",
        `Failed to compute uptime: ${uptimeResult.reason?.message}`,
      );
    }
    ...
    if (
      ...
      const extOnly = allServicesResult.value.data.filter(
        (s) => s.url && !s.url.startsWith(BACKEND_URL),
      );
      const uniqueWallets = new Set(
        extOnly
          .map((s) => (s.owner_address || "").toLowerCase())
          .filter(Boolean),
      );
      ...
      logger.warn(
        "PublicStats",
        `Failed to count external providers: ${allServicesResult.reason?.message}`,
      );
    }
    ...
    let usdcVolume = 0;
    if (
      usdcPaymentsResult.status === "fulfilled" &&
      usdcPaymentsResult.value.data
    ) {
      ...
      logger.warn(
        "PublicStats",
        `Failed to compute USDC volume: ${usdcPaymentsResult.reason?.message}`,
      );
    }
  ...
  router.get("/api/status/history", statsLimiter, async (req, res) => {
    ...
    if (!endpoint) {
      return res.status(400).json({
        error:
          "Parameter 'endpoint' required. Ex: /api/status/history?endpoint=/api/weather",
      });
    }
