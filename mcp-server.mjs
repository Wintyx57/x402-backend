...
function getPrivateKey() {
    ...
    console.error(`[Wallet] Tip: Use SKALE on Base for ultra-low gas (~$0.0007/tx). Pass chain: "skale" to any paid tool.`);
    return privateKey;
}
...
server.tool(
    ...
    async () => {
        try {
            ...
            // Fetch SKALE balance alongside default chain
            let skaleBalance = null;
            let skaleBalanceError = null;
            if (DEFAULT_CHAIN_KEY !== 'skale') {
                try {
                    const skaleCfg = CHAINS['skale'];
                    const { public: skalePubClient } = getClients('skale');
                    const skaleRaw = await skalePubClient.readContract({
                        address: skaleCfg.usdc,
                        abi: USDC_ABI,
                        functionName: 'balanceOf',
                        args: [account.address],
                    });
                    skaleBalance = (Number(skaleRaw) / 1e6).toFixed(6);
                } catch (err) {
                    skaleBalanceError = err.message;
                }
            }
            ...
            return {
                content: [{
                    ...
                    text: JSON.stringify({
                        ...
                        skale_usdc_balance: skaleBalance !== null ? `${skaleBalance} USDC` : null,
                        skale_balance_error: skaleBalanceError,
                        ...
                        wallet_backup_info: {
                            ...
                            warning: 'This wallet is encrypted with a key derived from your machine. Use export_private_key to backup.',
                        },
                    }, null, 2),
                }],
            };
...
server.tool(
    'export_private_key',
    'Export your wallet private key for backup or to import into MetaMask. WARNING: This reveals your private key — handle with extreme care. Never share it with anyone. Free — no payment needed.',
    {
            ...
            return {
                content: [{ type: 'text', text: JSON.stringify({
                    ...
                    instructions: [
                        ...
                        'You can import it into MetaMask: Settings → Import Account → Paste private key.',
