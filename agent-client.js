...
async function payAndRetry(wallet, requestFn, paymentDetails) {
  ...
  console.log(
    `    → Paiement de ${amount} USDC vers ${recipient.slice(0, 10)}...`,
  );
...
async function main() {
  ...
  Object.entries(home.endpoints).forEach(([k, v]) =>
    console.log(`       ${k} → ${v}`),
  );
  ...
  try {
    ...
    console.log(
      `    → Faucet USDC rate-limité, auto-financement via wallet serveur...`,
    );
    ...
    console.log(
      `    → 1.15 USDC envoyés depuis le wallet serveur (tx: ${fundConfirmed.getTransactionHash().slice(0, 20)}...)`,
    );
  }
  ...
  if (resSearch1.status === 402) {
    console.log(
      `    → HTTP 402 : ${bodySearch1.payment_details.action} coûte ${bodySearch1.payment_details.amount} USDC`,
    );
    ...
    if (searchResult.data && searchResult.data.length > 0) {
      searchResult.data.forEach((s) =>
        console.log(`       - ${s.name} (${s.price_usdc} USDC)`),
      );
    } else {
  ...
  if (resReg1.status === 402) {
    console.log(
      `    → HTTP 402 : ${bodyReg1.payment_details.action} coûte ${bodyReg1.payment_details.amount} USDC`,
    );
  ...
  resSearch2.data.forEach((s) =>
    console.log(
      `       - ${s.name} (${s.price_usdc} USDC) [ID ${s.id.slice(0, 8)}]`,
    ),
  );
  ...
  console.log(`\n=== RÉSUMÉ ===`);
  console.log(
    `Dépenses : recherche x2 (0.10 USDC) + enregistrement x1 (1.00 USDC) = 1.10 USDC`,
  );
