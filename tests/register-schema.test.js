...
describe('ServiceRegistrationSchema — cas nominal', () => {
    ...
    it('should accept price = 0 (gratuit)', () => {
        const result = ServiceRegistrationSchema.safeParse(validPayload({ price: 0 }));
        assert.strictEqual(result.success, true);
    });

    it('should accept price = 1000 (maximum)', () => {
        const result = ServiceRegistrationSchema.safeParse(validPayload({ price: 1000 }));
        assert.strictEqual(result.success, true);
    });

    it('should accept a name with exactly 1 character (minimum)', () => {
        const result = ServiceRegistrationSchema.safeParse(validPayload({ name: 'X' }));
        assert.strictEqual(result.success, true);
    });

    it('should accept a name with exactly 200 characters (maximum)', () => {
        const result = ServiceRegistrationSchema.safeParse(validPayload({ name: 'a'.repeat(200) }));
        assert.strictEqual(result.success, true);
    });

    it('should accept an optional description', () => {
        const result = ServiceRegistrationSchema.safeParse(validPayload({ description: 'A useful API' }));
        assert.strictEqual(result.success, true);
    ...
    it('should trim whitespace from name', () => {
        const result = ServiceRegistrationSchema.safeParse(validPayload({ name: '  My API  ' }));
        assert.strictEqual(result.success, true);
...
describe('ServiceRegistrationSchema — validation du prix', () => {
    it('should reject price above 1000', () => {
        const result = ServiceRegistrationSchema.safeParse(validPayload({ price: 1000.01 }));
        assert.strictEqual(result.success, false);
        assert.ok(firstIssueMessage(result).includes('1000'), `expected "1000" in message: ${firstIssueMessage(result)}`);
    });

    it('should reject price = 1001', () => {
        const result = ServiceRegistrationSchema.safeParse(validPayload({ price: 1001 }));
        assert.strictEqual(result.success, false);
    });

    it('should reject negative price', () => {
        const result = ServiceRegistrationSchema.safeParse(validPayload({ price: -0.001 }));
        assert.strictEqual(result.success, false);
        assert.ok(firstIssueMessage(result).includes('0'), `expected "0" in message: ${firstIssueMessage(result)}`);
    });

    it('should reject price = -1', () => {
        const result = ServiceRegistrationSchema.safeParse(validPayload({ price: -1 }));
        assert.strictEqual(result.success, false);
    });

    it('should reject price as string', () => {
        const result = ServiceRegistrationSchema.safeParse(validPayload({ price: '0.01' }));
        assert.strictEqual(result.success, false);
    });

    it('should reject price as null', () => {
        const result = ServiceRegistrationSchema.safeParse(validPayload({ price: null }));
        assert.strictEqual(result.success, false);
    });
...
describe('ServiceRegistrationSchema — validation du nom', () => {
    it('should reject empty name', () => {
        const result = ServiceRegistrationSchema.safeParse(validPayload({ name: '' }));
        assert.strictEqual(result.success, false);
        assert.ok(firstIssueMessage(result).toLowerCase().includes('required') ||
    ...
    it('should reject name with 201 characters', () => {
        const result = ServiceRegistrationSchema.safeParse(validPayload({ name: 'a'.repeat(201) }));
        assert.strictEqual(result.success, false);
...
describe('ServiceRegistrationSchema — validation de l\'URL', () => {
    it('should accept https URL', () => {
        const result = ServiceRegistrationSchema.safeParse(validPayload({ url: 'https://api.example.com/v2/data' }));
        assert.strictEqual(result.success, true);
    });
    ...
    it('should accept http URL (schema ne restreint pas au https)', () => {
        ...
        const result = ServiceRegistrationSchema.safeParse(validPayload({ url: 'http://example.com/api' }));
        assert.strictEqual(result.success, true);
    });

    it('should reject a non-URL string', () => {
        const result = ServiceRegistrationSchema.safeParse(validPayload({ url: 'not-a-url' }));
        assert.strictEqual(result.success, false);
    });

    it('should reject URL longer than 500 characters', () => {
        const longUrl = 'https://example.com/' + 'a'.repeat(490);
        const result = ServiceRegistrationSchema.safeParse(validPayload({ url: longUrl }));
        assert.strictEqual(result.success, false);
    });

    it('should reject empty URL', () => {
        const result = ServiceRegistrationSchema.safeParse(validPayload({ url: '' }));
        assert.strictEqual(result.success, false);
    });
    ...
    it('should reject ftp:// URL (not http/https)', () => {
        // Zod z.string().url() accepte ftp — comportement documenté ici
        // (restriction applicative gérée dans la route, pas le schema Zod pur)
        const result = ServiceRegistrationSchema.safeParse(validPayload({ url: 'ftp://example.com/api' }));
        // Ce test documente le comportement actuel : Zod l'accepte
        assert.strictEqual(result.success, true, 'Le schéma Zod pur accepte ftp (restriction dans la route)');
    });
});
...
describe('ServiceRegistrationSchema — validation ownerAddress', () => {
    it('should accept a valid checksummed address', () => {
        const result = ServiceRegistrationSchema.safeParse(
            validPayload({ ownerAddress: '0xfb1c478BD5567BdcD39782E0D6D23418bFda2430' })
        );
        assert.strictEqual(result.success, true);
    });
    ...
    it('should reject an address without 0x prefix', () => {
        ...
        assert.strictEqual(result.success, false);
        assert.ok(firstIssueMessage(result).toLowerCase().includes('address') ||
