            ...
            return {
                    ...
                    return {
                        select() {
                            if (mockData.insertError) {
                                return Promise.resolve({ data: null, error: mockData.insertError });
                            }
                            const rows = mockData.insertedRows
                                || (Array.isArray(data)
                ...
                select(fields) {
                    ...
                    directResult.eq = function(col, val) {
                        return {
                            order(col2, opts) {
                                if (mockData.pendingError) {
                                    return Promise.resolve({ data: null, error: mockData.pendingError });
                                }
                                return Promise.resolve({ data: mockData.pendingPayouts || [], error: null });
                            }
                        };
                    };
                            ...
                            return {
                                select() {
                                    if (mockData.updateError) {
                                        return Promise.resolve({ data: null, error: mockData.updateError });
                                    }
                                    const rows = (mockData.updatedRows || []).map(r => ({ ...r, ...updateData }));
                                    return Promise.resolve({ data: rows, error: null });
                                }
                            };
                        }
    ...
    it('should return null (not throw) when supabase insert rejects', async () => {
        const supabase = {
            from() {
                return {
                    insert() {
                        return { select: () => Promise.reject(new Error('Network timeout')) };
                    }
                };
            }
        };
        ...
        assert.ok(!threw || result === null || result === undefined,
...
describe('getPendingPayouts', () => {
    ...
    it('should group payouts by provider wallet and compute per-provider totals', async () => {
        ...
        const pendingPayouts = [
            makePayout({ id: 'p1', provider_wallet: walletA, provider_amount: 0.95, platform_fee: 0.05 }),
            makePayout({ id: 'p2', provider_wallet: walletA, provider_amount: 1.90, platform_fee: 0.10 }),
            makePayout({ id: 'p3', provider_wallet: walletB, provider_amount: 9.50, platform_fee: 0.50 }),
        ];
        ...
        assert.ok(Math.abs(providerA.total_owed  - 2.85) < 1e-10,
            `walletA total_owed: expected 2.85, got ${providerA.total_owed}`);
        assert.ok(Math.abs(providerA.total_fees  - 0.15) < 1e-10,
            `walletA total_fees: expected 0.15, got ${providerA.total_fees}`);

        // Wallet B: 1 payout
        assert.equal(providerB.count, 1);
        assert.ok(Math.abs(providerB.total_owed  - 9.50) < 1e-10);
        assert.ok(Math.abs(providerB.total_fees  - 0.50) < 1e-10);
    });
    ...
    it('should compute correct global summary totals across all providers', async () => {
        ...
        const pendingPayouts = [
            makePayout({ id: 'p1', provider_wallet: walletA, provider_amount: 0.95, platform_fee: 0.05 }),
            makePayout({ id: 'p2', provider_wallet: walletB, provider_amount: 9.50, platform_fee: 0.50 }),
        ];
...
describe('markPayoutsPaid', () => {
    ...
    it('should pass status "paid" and tx_hash_out to the update call', async () => {
            ...
            from() {
                return {
                    update(data) {
                        ...
                        return {
                            in() {
                                return { select: () => Promise.resolve({ data: [{ id: 'p1', ...data }], error: null }) };
                            }
                        };
                    }
                };
        ...
        assert.ok(!isNaN(Date.parse(capturedUpdateData.paid_at)), 'paid_at should be a valid ISO date');
    });
...
describe('getRevenueOverview', () => {
    ...
    it('should sum gross, provider and fee amounts across all rows', async () => {
        const overviewRows = [
            { gross_amount: '1.00', provider_amount: '0.95', platform_fee: '0.05', status: 'paid',    chain: 'base' },
            { gross_amount: '2.00', provider_amount: '1.90', platform_fee: '0.10', status: 'pending', chain: 'base' },
            { gross_amount: '0.10', provider_amount: '0.095',platform_fee: '0.005',status: 'paid',    chain: 'skale' },
        ];
        ...
        assert.ok(Math.abs(overview.total_gross            - 3.10)  < 1e-10, `total_gross: ${overview.total_gross}`);
        assert.ok(Math.abs(overview.total_provider_payouts - 2.945) < 1e-10, `total_provider_payouts: ${overview.total_provider_payouts}`);
        assert.ok(Math.abs(overview.total_platform_fees    - 0.155) < 1e-10, `total_platform_fees: ${overview.total_platform_fees}`);
    });

    it('should segregate pending vs paid provider amounts', async () => {
        const overviewRows = [
            { gross_amount: '10', provider_amount: '9.5', platform_fee: '0.5', status: 'paid',    chain: 'base' },
            { gross_amount: '2',  provider_amount: '1.9', platform_fee: '0.1', status: 'pending', chain: 'base' },
        ];
    ...
    it('should count rows by status in by_status', async () => {
        const overviewRows = [
            { gross_amount: '1', provider_amount: '0.95', platform_fee: '0.05', status: 'paid',    chain: 'base' },
            { gross_amount: '1', provider_amount: '0.95', platform_fee: '0.05', status: 'paid',    chain: 'base' },
            { gross_amount: '1', provider_amount: '0.95', platform_fee: '0.05', status: 'pending', chain: 'base' },
            { gross_amount: '1', provider_amount: '0.95', platform_fee: '0.05', status: 'failed',  chain: 'base' },
        ];
    ...
    it('should aggregate gross amounts by chain in by_chain', async () => {
        const overviewRows = [
            { gross_amount: '1',  provider_amount: '0.95', platform_fee: '0.05', status: 'paid',    chain: 'base'  },
            { gross_amount: '2',  provider_amount: '1.90', platform_fee: '0.10', status: 'paid',    chain: 'base'  },
            { gross_amount: '0.5',provider_amount: '0.475',platform_fee: '0.025',status: 'pending', chain: 'skale' },
        ];
        ...
        assert.ok(Math.abs(overview.by_chain['base']  - 3.0) < 1e-10, `by_chain.base: ${overview.by_chain['base']}`);
        assert.ok(Math.abs(overview.by_chain['skale'] - 0.5) < 1e-10, `by_chain.skale: ${overview.by_chain['skale']}`);
    });
