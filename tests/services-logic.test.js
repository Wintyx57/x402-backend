...
// Sanitization helpers are imported from lib/validators.js (single source of truth).
...
// Control character rejection regex — mirrors the Zod schema in schemas/index.js
...
describe("endpoint activity patterns", () => {
  ...
  it('should match "Scraper API" → /api/scrape', () => {
    assert.equal(
      matchEndpoint("Scraper API: url=https://example.com"),
      "/api/scrape",
    );
  });
  ...
  it('should match "Crypto Price API" → /api/crypto', () => {
    assert.equal(
      matchEndpoint("Crypto Price API: coin=bitcoin"),
      "/api/crypto",
    );
  });
  ...
  it('should match "Image Generation API" → /api/image', () => {
    assert.equal(
      matchEndpoint("Image Generation API: prompt=cat"),
      "/api/image",
    );
  });
