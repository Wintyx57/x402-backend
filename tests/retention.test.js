                ...
                delete(opts) {
                    ...
                    return {
                        lt(col, cutoff) {
                            ...
                            return Promise.resolve({ error: null, count: options.deletedCount || 0 });
                        },
                    };
                },
            };
...
async function purgeOldData(supabase) {
    ...
    try {
        ...
        await supabase.from('activity').delete({ count: 'exact' }).lt('created_at', activityCutoff);
    } catch (_) {}

    try {
        const monitorCutoff = new Date(Date.now() - MONITORING_RETENTION_DAYS * 86400 * 1000).toISOString();
        await supabase.from('monitoring_checks').delete({ count: 'exact' }).lt('checked_at', monitorCutoff);
    } catch (_) {}

    try {
        const dailyCutoff = new Date(Date.now() - DAILY_CHECKS_RETENTION_DAYS * 86400 * 1000).toISOString();
        await supabase.from('daily_checks').delete({ count: 'exact' }).lt('checked_at', dailyCutoff);
    } catch (_) {}
}
...
describe('retention — cutoff date calculations', () => {
    it('activity cutoff should be ~90 days ago', () => {
        ...
        assert.ok(Math.abs(daysAgo - 90) < 0.01, `Expected ~90 days, got ${daysAgo}`);
    });
    ...
    it('activity cutoff should be a valid ISO 8601 string', () => {
        ...
        assert.ok(cutoff.includes('T'), 'Should contain T separator for ISO format');
    });
...
describe('retention — purgeOldData calls', () => {
    it('should call delete on activity, monitoring_checks, and daily_checks', async () => {
        ...
        assert.ok(tables.includes('monitoring_checks'), 'Should call monitoring_checks');
    ...
    it('should continue processing other tables after one error', async () => {
        ...
        assert.ok(processedTables.includes('monitoring_checks'), 'monitoring_checks should still be processed');
        assert.ok(processedTables.includes('daily_checks'), 'daily_checks should still be processed');
    });
});
