...
function makeSuccessfulRpcFetch(recipientAddr, amountRaw) {
  const paddedRecipient =
    "0x000000000000000000000000" + recipientAddr.slice(2).toLowerCase();
...
describe("verifySplitPayment — exported function", () => {
  ...
  it("should return { providerValid: true, platformValid: false, fromAddress } when only provider tx is valid", async () => {
    ...
    const result = await system.verifySplitPayment(
      TX_HASH_PROVIDER,
      null,
      1000,
      "base",
    ...
    assert.equal(
      result.platformValid,
      false,
      "Platform should be false when txHashPlatform is null",
    );
    assert.ok(
      typeof result.fromAddress === "string",
      "fromAddress should be a string",
    );
  });
  ...
  it("should return { providerValid: true, platformValid: false } when platform tx is invalid", async () => {
    ...
    global.fetch = async (url, opts) => {
      ...
      if (callIndex === 1) {
        // Provider tx: valid
        return makeSuccessfulRpcFetch(PROVIDER_WALLET, providerAmountRaw)(
          url,
          opts,
        );
      }
  ...
  it("should use Math.floor for provider amount and subtraction for platform amount", async () => {
    ...
    global.fetch = async (url, opts) => {
      ...
      if (receiptCallCount === 1) {
        ...
        return makeSuccessfulRpcFetch(PROVIDER_WALLET, capturedProviderAmount)(
          url,
          opts,
        );
      }
      capturedPlatformAmount = total - capturedProviderAmount;
      return makeSuccessfulRpcFetch(PLATFORM_WALLET, capturedPlatformAmount)(
        url,
        opts,
      );
    };
    ...
    const system = createPaymentSystem(fakeSupabase, () => {});
    await system.verifySplitPayment(
      TX_HASH_PROVIDER,
      TX_HASH_PLATFORM,
      total,
      "base",
...
describe("recordSplitPayout", () => {
  ...
  it("should insert a row with split_mode = split_complete and status = paid", async () => {
    ...
    assert.equal(
      capturedRow.status,
      "paid",
      "split_complete should produce status = paid immediately",
    );
  ...
  it("should insert a row with split_mode = provider_only and status = pending", async () => {
    ...
    assert.equal(
      capturedRow.status,
      "pending",
      "provider_only should remain pending (platform owes its share)",
    );
  ...
  it("should compute correct 95/5 split for 0.01 USDC", async () => {
    ...
    assert.ok(
      Math.abs(capturedRow.provider_amount - 0.0095) < 1e-10,
      `provider_amount: ${capturedRow.provider_amount}`,
    );
    assert.ok(
      Math.abs(capturedRow.platform_fee - 0.0005) < 1e-10,
      `platform_fee: ${capturedRow.platform_fee}`,
    );
    assert.ok(
      Math.abs(capturedRow.gross_amount - 0.01) < 1e-10,
      `gross_amount: ${capturedRow.gross_amount}`,
    );
  });
  ...
  it("should compute correct 95/5 split for 1.0 USDC", async () => {
    ...
    assert.ok(
      Math.abs(capturedRow.provider_amount - 0.95) < 1e-10,
      `provider_amount: ${capturedRow.provider_amount}`,
    );
    assert.ok(
      Math.abs(capturedRow.platform_fee - 0.05) < 1e-10,
      `platform_fee: ${capturedRow.platform_fee}`,
    );
  });
  ...
  it("should store tx_hash_provider in tx_hash_in column", async () => {
    ...
    assert.equal(
      capturedRow.tx_hash_in,
      TX_HASH_PROVIDER,
      "tx_hash_in should be the provider tx hash",
    );
    assert.equal(
      capturedRow.tx_hash_platform,
      TX_HASH_PLATFORM,
      "tx_hash_platform should be stored separately",
    );
...
function makeReqRes({ headers = {}, body = {}, params = {} } = {}) {
  ...
  const res = {
    ...
    getResponse() {
      return { statusCode: _statusCode, body: _responseBody };
    },
    setHeader() {
      return res;
    },
...
function makeAutoApprovePaymentMiddleware() {
  return (minAmountRaw, price, label, options = {}) =>
    (req, res, next) => {
...
function makeSplitPaymentSystem({
  providerValid = true,
  platformValid = true,
  fromAddress = "0x" + "f".repeat(40),
} = {}) {
  return {
    verifySplitPayment: async () => ({
      providerValid,
      platformValid,
      fromAddress,
    }),
...
describe("proxy.js — input validation (split mode)", () => {
  ...
  it("should reject 400 when txHashProvider and txHashPlatform are identical", async () => {
    ...
    assert.ok(
      body.message.includes("different"),
      `Expected "different" in: ${body.message}`,
    );
  });
  ...
  it("should reject 400 when price < 100 micro-USDC and split mode is used", async () => {
    ...
    assert.equal(statusCode, 400);
    assert.ok(
      body.error.includes("too low"),
      `Expected "too low" in: ${body.error}`,
    );
  });
});
...
describe("proxy.js — anti-replay (split mode)", () => {
  ...
  it("should reject 409 when txHashProvider is already in used_transactions", async () => {
    ...
    assert.ok(
      ...
      `Expected "already been used" in: ${body.message}`,
    );
  });
...
describe("proxy.js — on-chain verification (split mode)", () => {
  ...
  it("should reject 402 when provider payment is invalid", async () => {
    ...
    assert.ok(
      body.message.includes("Provider payment invalid"),
      `Expected "Provider payment invalid" in: ${body.message}`,
    );
  });
  ...
  it("should serve 200 when provider tx valid and platform tx absent (provider_only mode)", async () => {
    ...
    assert.equal(
      payoutMock.calls.recordSplitPayout[0].splitMode,
      "provider_only",
    );
  });
        ...
        if (table === "services") {
          return {
            select: () => ({
              eq: () => ({
                single: () =>
                  Promise.resolve({ data: SERVICE_WITH_OWNER, error: null }),
              }),
            }),
          };
        }
...
describe("proxy.js — legacy mode", () => {
  ...
  it("should use legacy mode when service has no owner_address", async () => {
    ...
    assert.equal(statusCode, 200);
    assert.ok(
      body._x402.tx_hash !== undefined,
      "_x402.tx_hash should be set in legacy mode",
    );
    assert.ok(
      body._x402.split_mode === undefined,
      "_x402.split_mode should NOT be set in legacy mode",
    );
    assert.equal(
      payoutMock.calls.recordSplitPayout.length,
      0,
      "Should NOT call recordSplitPayout",
    );
  });
  ...
  it("should use legacy mode when service has owner_address but client sends X-Payment-TxHash (not split headers)", async () => {
    ...
    assert.equal(
      payoutMock.calls.recordSplitPayout.length,
      0,
      "recordSplitPayout should NOT be called in legacy mode",
    );
    assert.equal(
      payoutMock.calls.recordPayout.length,
      1,
      "recordPayout SHOULD be called in legacy mode",
    );
  });
  ...
  it("should enrich 402 response with provider_wallet + split info when service has owner_address (legacy mode)", async () => {
    ...
    const alwaysReject402 =
      (minAmountRaw, price, label) => (req, res, next) => {
        res.status(402).json({
    ...
    assert.equal(statusCode, 402);
    assert.ok(
      body.payment_details.provider_wallet,
      "provider_wallet should be set",
    );
    ...
    assert.ok(
      body.payment_details.split.provider_amount > 0,
      "provider_amount should be > 0",
    );
    assert.ok(
      body.payment_details.split.platform_amount > 0,
      "platform_amount should be > 0",
    );
  });
  ...
  it("should NOT add provider_wallet to 402 response when service has no owner_address", async () => {
    ...
    const { req, res } = makeReqRes({
      ...
      headers: {
        "x-payment-chain": "base",
        "x-payment-txhash": "0x" + "bb".repeat(32),
      },
    });
    ...
    assert.equal(
      body.payment_details.provider_wallet,
      undefined,
      "No provider_wallet for native service",
    );
    assert.equal(
      body.payment_details.split,
      undefined,
      "No split info for native service",
    );
  });
});
...
describe("proxy.js — deferred claiming (upstream failures)", () => {
  ...
  it("should NOT claim tx when upstream returns 5xx after all retries (split mode)", async () => {
    ...
    assert.equal(
      body._x402.retry_eligible,
      true,
      "Should indicate retry is eligible",
    );
    assert.ok(
      body.message.includes("NOT consumed"),
      `Expected "NOT consumed" in: ${body.message}`,
    );
    assert.equal(
      insertCalled,
      false,
      "TX should NOT be claimed when upstream fails",
    );
  });
  ...
  it("should NOT claim tx when upstream is unreachable (network error, split mode)", async () => {
    ...
    assert.equal(
      insertCalled,
      false,
      "TX should NOT be claimed on network error",
    );
  });
  ...
  it("should NOT claim tx when upstream returns 4xx (consumer protection)", async () => {
    ...
    assert.equal(
      payoutMock.calls.recordSplitPayout.length,
      0,
      "TX should NOT be claimed on 4xx (consumer protection)",
    );
    assert.equal(
      body._payment_status,
      "not_charged",
      "Response should include _payment_status: not_charged",
    );
    assert.equal(
      body._x402.retry_eligible,
      true,
      "TX should be retry eligible",
    );
  });
  ...
  it("should retry on 5xx and succeed on 2nd attempt", async () => {
    ...
    assert.equal(
      payoutMock.calls.recordSplitPayout.length,
      1,
      "TX should be claimed after successful retry",
    );
  });
  ...
  it("should NOT claim tx when upstream fails in legacy mode", async () => {
    ...
    assert.equal(
      payoutMock.calls.recordPayout.length,
      0,
      "Should NOT record payout when upstream fails",
    );
  });
});
